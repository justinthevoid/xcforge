...
enum TestRenderer {
    static func renderTest(_ execution: TestTools.TestExecution) -> String {
        ...
        if execution.expectedFailureCount > 0 { statParts.append("\(execution.expectedFailureCount) expected-failure") }
    ...
    static func renderFailures(_ result: TestTools.TestFailuresResult) -> String {
        ...
        for failure in result.failures {
            ...
            if let console = result.consoleByTest[funcName] ?? result.consoleByTest[failure.testIdentifier] {
    ...
    static func renderListTests(_ result: TestTools.ListTestsResult) -> String {
        var lines: [String] = []
        lines.append("\(result.testCount) tests in \(result.targetCount) target(s), \(result.classCount) class(es)")
        lines.append("")
    ...
    static func renderFileCoverage(_ detail: TestTools.FileCoverageDetail) -> String {
        ...
        for fn in detail.functions {
            if fn.executionCount == 0 {
                lines.append(String(format: "  L%-4d %-40s   0%%  UNTESTED  (%d lines)", fn.lineNumber, fn.name, fn.executableLines))
                untested.append("\(fn.name) (L\(fn.lineNumber), \(fn.executableLines) lines)")
            } else {
                lines.append(String(format: "  L%-4d %-40s %3.0f%%  (%dx called)", fn.lineNumber, fn.name, fn.lineCoverage * 100, fn.executionCount))
            }
        }
