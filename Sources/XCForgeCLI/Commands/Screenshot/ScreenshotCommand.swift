...
struct ScreenshotCapture: ParsableCommand {
    static let configuration = CommandConfiguration(
        commandName: "capture",
...
struct ScreenshotBaseline: ParsableCommand {
    static let configuration = CommandConfiguration(
    ...
    mutating func run() throws {
        let name = self.name
        let simulator = self.simulator
        let baselineDir = self.baselineDir
        let json = self.json
...
struct ScreenshotCompare: ParsableCommand {
    static let configuration = CommandConfiguration(
        commandName: "compare",
