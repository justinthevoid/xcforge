...
enum DiagnosisFinalResultRenderer {
  ...
  static func render(_ result: DiagnosisFinalResult, layout: TerminalLayout) -> String {
    ...
    if let currentAttempt = result.currentAttempt {
      appendAttemptBlock(
        &lines, title: "Current State", attempt: currentAttempt, includeContext: true,
        layout: layout)
      appendDiagnosisDetail(&lines, attempt: currentAttempt, phase: result.phase)
      appendEvidenceBundle(
        &lines, title: "Current Evidence Bundle", attempt: currentAttempt, layout: layout)
    ...
    if let sourceAttempt = result.sourceAttempt {
      ...
      appendEvidenceBundle(
        &lines, title: "Prior Evidence Bundle", attempt: sourceAttempt, layout: layout)
    }
  ...
  private static func appendInvestigationSummary(
    ...
    lines.append(
      "  evidence: \(WorkflowPresentationHelpers.evidenceSummary(for: result.currentAttempt))")
    lines.append(
      "  proof_cue: \(WorkflowPresentationHelpers.primaryProofCue(for: result.currentAttempt))")
  ...
  private static func buildMajorActions(for result: DiagnosisFinalResult) -> [String] {
    ...
    if let sourceAttemptId = result.sourceAttemptId {
      actions.append(
        "Linked this terminal result to source attempt \(sourceAttemptId) for rerun review.")
