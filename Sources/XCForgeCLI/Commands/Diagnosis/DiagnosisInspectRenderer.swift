...
enum DiagnosisInspectRenderer {
  static func render(_ result: DiagnosisInspectResult) -> String {
    ...
    if !result.actionHistory.isEmpty {
      ...
      lines += result.actionHistory.map { action in
        var line =
          "  [\(formatter.string(from: action.timestamp))] \(action.kind.rawValue) (\(action.phase.rawValue))"
  ...
  private static func appendContextProvenance(
    _ lines: inout [String], _ provenance: WorkflowContextProvenance
  ) {
  ...
  private static func renderEvidenceLine(_ record: WorkflowEvidenceRecord) -> String {
    ...
    return
      "  - \(record.kind.rawValue) | phase=\(record.phase.rawValue) | attempt=\(record.attemptNumber) | source=\(record.source) | reference=\(reference)"
  }

  private static func renderUnavailableEvidenceLine(_ record: WorkflowEvidenceRecord) -> String {
    var line =
      "  - \(record.kind.rawValue) | phase=\(record.phase.rawValue) | attempt=\(record.attemptNumber)"
