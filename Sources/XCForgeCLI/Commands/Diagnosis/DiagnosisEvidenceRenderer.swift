...
enum DiagnosisEvidenceRenderer {
  ...
  private static func appendRuntimeEvidence(
    _ lines: inout [String], _ summary: RuntimeDiagnosisSummary
  ) {
    lines.append("Runtime evidence:")
    lines.append("  observed: \(summary.observedEvidence.summary)")
    lines.append(
  ...
  private static func renderArtifactLine(_ record: WorkflowEvidenceRecord) -> String {
    ...
    return
      "  - \(record.kind.rawValue) | phase=\(record.phase.rawValue) | attempt=\(record.attemptNumber) | state=\(record.availabilityLabel) | source=\(record.source) | reference=\(reference)"
  }

  private static func renderMissingArtifactLine(_ record: WorkflowEvidenceRecord) -> String {
    var line =
      "  - \(record.kind.rawValue) | phase=\(record.phase.rawValue) | attempt=\(record.attemptNumber) | state=\(record.availabilityLabel)"
  ...
  private static func shouldExplainMissingRuntimeEvidence(for result: DiagnosisEvidenceResult)
    -> Bool
  {
    result.runtimeSummary == nil
      && (result.phase == .diagnosisRuntime || result.evidenceState == .empty)
  }
}
