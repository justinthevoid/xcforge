...
enum DiagnosisCompareRenderer {
  ...
  private static func renderMissingArtifactLine(_ record: WorkflowEvidenceRecord, indent: String)
    -> String
  {
    var line =
      "\(indent)- \(record.kind.rawValue) | phase=\(record.phase.rawValue) | attempt=\(record.attemptNumber) | state=\(record.availabilityLabel)"
