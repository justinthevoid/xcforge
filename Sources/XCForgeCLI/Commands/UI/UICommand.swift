  ...
  mutating func run() async throws {
    ...
    if healthy {
      do {
        ...
        if json {
          let result = UIResult(
            succeeded: true, message: message, elementId: nil, elementCount: nil)
    ...
    do {
      ...
      if json {
        print(
          try WorkflowJSONRenderer.renderJSON(
            UIResult(
              succeeded: true, message: message, elementId: nil, elementCount: elements.count)))
      } else {
        print(message)
      }
    } catch {
...
struct UIPinch: AsyncParsableCommand {
  ...
  mutating func run() async throws {
    let env = Environment.live
    do {
      try await env.wdaClient.pinch(
        centerX: centerX, centerY: centerY, scale: scale, durationMs: durationMs)
...
struct UIAlert: AsyncParsableCommand {
  ...
  @Option(
    help: "Specific button to tap (e.g. 'Allow While Using App'). If omitted, uses smart defaults.")
  ...
  mutating func run() async throws {
    ...
    if json {
      print(
        try WorkflowJSONRenderer.renderJSON(
          UIResult(succeeded: succeeded, message: message, elementId: nil, elementCount: nil)))
    } else {
      print(message)
    }
