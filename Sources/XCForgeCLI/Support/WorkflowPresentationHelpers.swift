...
enum WorkflowPresentationHelpers {
  ...
  static func primaryProofCue(for attempt: DiagnosisCompareAttemptSnapshot?) -> String {
    ...
    if let artifact = preferredProofArtifact(from: attempt.availableEvidence) {
      return
        "Best available artifact: \(humanize(artifact.kind.rawValue)) from attempt \(artifact.attemptNumber)."
    }

    if let missing = attempt.unavailableEvidence.first {
      var cue =
        "Best available artifact is missing: \(humanize(missing.kind.rawValue)) for attempt \(missing.attemptNumber)"
  ...
  static func formatEvidenceRecord(
    _ record: WorkflowEvidenceRecord, layout: TerminalLayout, indent: String = "    "
  ) -> String {
    ...
    case .medium:
      return
        "\(indent)- \(record.kind.rawValue) | phase=\(record.phase.rawValue) | attempt=\(record.attemptNumber)"
        + "\n\(indent)  state=\(record.availabilityLabel) | source=\(source) | reference=\(reference)"
    case .wide:
      return
        "\(indent)- \(record.kind.rawValue) | phase=\(record.phase.rawValue) | attempt=\(record.attemptNumber) | state=\(record.availabilityLabel) | source=\(source) | reference=\(reference)"
    }
  }

  static func formatMissingEvidenceRecord(
    _ record: WorkflowEvidenceRecord, layout: TerminalLayout, indent: String = "    "
  ) -> String {
  ...
  static func formatRecoveryRecord(
    _ recovery: WorkflowRecoveryRecord, layout: TerminalLayout, indent: String = "  "
  ) -> String {
    switch layout {
    case .narrow:
      var lines = [
    ...
    case .medium:
      let line1 =
        "\(indent)- \(recovery.recoveryId) | issue=\(recovery.issue.label) | action=\(recovery.action.label)"
      var line2 =
        "\(indent)  status=\(recovery.status.rawValue) | resumed=\(recovery.resumed ? "yes" : "no")"
