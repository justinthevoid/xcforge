...
public enum TestTools {
        ...
        Tool(
            ...
            inputSchema: .object([
                ...
                "properties": .object([
                    ...
                    "filter": .object(["type": .string("string"), "description": .string("Test filter. Accepts: 'testMethod', 'TestClass/testMethod', or full 'TestTarget/TestClass/testMethod'. The test target prefix is auto-resolved if omitted. Use list_tests to discover available test identifiers.")]),
    ...
    static func zeroMatchHint(
        filter: String, project: String?, scheme: String?, simulator: String?, env: Environment
    ) async -> String {
        ...
        hint += " (found \(listResult.testCount) tests in bundle)"
        if listResult.testCount == 0 {
            hint += "\nThe test bundle appears to be empty. Check that test targets compile and contain test methods."
            return hint
        }
                   ...
                   let json = try? JSONSerialization.jsonObject(with: data) as? [String: Any] {
                    ...
                    if totalTests == 0, let f = input.filter {
                        summary += await zeroMatchHint(
                            ...
                            simulator: input.simulator, env: env
                        )
                    }
    ...
    static func formatListTests(_ result: ListTestsResult, filter: String? = nil) -> CallTool.Result {
        ...
        lines.append("  Class:  filter: \"Class\" (auto-resolves target)")
        lines.append("  Method: filter: \"method\" (auto-resolves target)")
