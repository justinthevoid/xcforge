...
enum UITools {
    public static let tools: [Tool] = [
        ...
        Tool(
            name: "indigo_tap",
            description: "Tap at x,y via native HID (sub-5ms, bypasses WDA). Falls back to WDA if unavailable.",
            inputSchema: .object([
                "type": .string("object"),
                "properties": .object([
        ...
        Tool(
            ...
            inputSchema: .object([
                "type": .string("object"),
                "properties": .object([
                    "start_x": .object(["type": .string("number"), "description": .string("Start X (points)")]),
                    "start_y": .object(["type": .string("number"), "description": .string("Start Y (points)")]),
    ...
    static func handleAlert(_ args: [String: Value]?, session: SessionState, wdaClient: WDAClient) async -> CallTool.Result {
        ...
        case "get_text":
            guard let info = await wdaClient.getAlertText() else {
                Task { guard let udid = await DeviceStateStore.currentUDID(session: session) else { return }
    ...
    static func wdaStatus(_ args: [String: Value]?, session: SessionState, wdaClient: WDAClient) async -> CallTool.Result {
        ...
        Task { guard let udid = await DeviceStateStore.currentUDID(session: session) else { return }
            await DeviceStateStore.shared.update(.wdaStatus(healthy ? "healthy (\(backendName))" : "not responding"), for: udid) }
    ...
    static func wdaCreateSession(_ args: [String: Value]?, wdaClient: WDAClient) async -> CallTool.Result {
    ...
    static func tapCoordinates(_ args: [String: Value]?, wdaClient: WDAClient) async -> CallTool.Result {
    ...
    static func swipeAction(_ args: [String: Value]?, wdaClient: WDAClient) async -> CallTool.Result {
        ...
        case .success(let input):
            ...
            do {
                try await wdaClient.swipe(startX: input.start_x, startY: input.start_y, endX: input.end_x, endY: input.end_y, durationMs: durationMs)
                return .ok("Swiped from (\(Int(input.start_x)),\(Int(input.start_y))) to (\(Int(input.end_x)),\(Int(input.end_y)))")
            } catch {
    ...
    static func pinchAction(_ args: [String: Value]?, wdaClient: WDAClient) async -> CallTool.Result {
        ...
        case .success(let input):
            ...
            do {
                try await wdaClient.pinch(centerX: input.center_x, centerY: input.center_y, scale: input.scale, durationMs: durationMs)
        ...
        case .success(let input):
            ...
            do {
                if let eid = input.element_id {
                    ...
                    let (eid, _) = try await wdaClient.findElement(using: "class name", value: "XCUIElementTypeTextField")
                    if clearFirst {
                        try await wdaClient.clearElement(elementId: eid)
                    }
                    try await wdaClient.setValue(elementId: eid, text: input.text)
        ...
        case .success(let input):
            ...
            if IndigoHIDClient.isAvailable {
                do {
                    ...
                    let elapsed = String(format: "%.1f", (CFAbsoluteTimeGetCurrent() - start) * 1000)
                    return .ok("Swiped from (\(Int(input.start_x)),\(Int(input.start_y))) to (\(Int(input.end_x)),\(Int(input.end_y))) via IndigoHID (\(elapsed)ms)")
                } catch {
            ...
            do {
                try await wdaClient.swipe(startX: input.start_x, startY: input.start_y, endX: input.end_x, endY: input.end_y, durationMs: durationMs)
                return .ok("Swiped from (\(Int(input.start_x)),\(Int(input.start_y))) to (\(Int(input.end_x)),\(Int(input.end_y))) via WDA fallback")
            } catch {
                return .fail("Swipe failed: \(error)")
            }
        }
        ...
        case .success(let input):
            ...
            if format == "json", AXPBridge.isAvailable {
                ...
                do {
                    ...
                    Task { guard let udid = await DeviceStateStore.currentUDID(session: env.session) else { return }
                        await DeviceStateStore.shared.update(.screen(elementCount: max(elementCount, 1), summary: "json"), for: udid) }
            ...
            do {
                ...
                let elementCount = source.components(separatedBy: "\"type\"").count - 1
                Task { guard let udid = await DeviceStateStore.currentUDID(session: env.session) else { return }
                    await DeviceStateStore.shared.update(.screen(elementCount: max(elementCount, 1), summary: format), for: udid) }
