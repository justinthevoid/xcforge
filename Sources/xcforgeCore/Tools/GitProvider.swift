...
enum GitTools {
    ...
    static func gitStatus(_ args: [String: Value]?, env: Environment) async -> CallTool.Result {
        ...
        case .success(let input):
            do {
                let result = try await env.shell.git(["status", "--porcelain"], workingDirectory: input.path)
    ...
    static func gitCommit(_ args: [String: Value]?, env: Environment) async -> CallTool.Result {
        ...
        case .success(let input):
            do {
                ...
                let result = try await env.shell.git(["commit", "-m", input.message], workingDirectory: input.path)
                if result.succeeded {
