...
public actor WDAClient {
    ...
    private func cleanupWDAProcesses(simulator: String) async {
        let _ = try? await Shell.xcrun(timeout: 5, "simctl", "terminate", simulator, WDABackend.xcForgeWDA.bundleId)
    ...
    public func getElementRect(elementId: String) async throws -> ElementRect {
        let sid = try await ensureSession()
        let json = try await jsonRequest(method: "GET", path: "/session/\(sid)/element/\(elementId)/rect")
        guard let value = json["value"] as? [String: Any],
              let x = value["x"] as? Double,
              let y = value["y"] as? Double,
              let w = value["width"] as? Double,
              let h = value["height"] as? Double else {
...
enum WDAError: Error, CustomStringConvertible {
    ...
    var description: String {
        ...
        case .noBackendAvailable: return "No WDA backend available. Neither xcforgeWDA nor Original WDA could be started. Install xcforgeWDA or start WebDriverAgent."
        }
    }
}
