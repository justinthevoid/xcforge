...
public struct StepResult: Codable, Sendable {
    ...
    public init(index: Int, type: String, status: StepStatus, detail: String?, durationMs: Int, screenshotBase64: String? = nil) {
        self.index = index
        self.type = type
        self.status = status
        self.detail = detail
...
public struct PlanReport: Codable, Sendable {
    ...
    public static func merge(prior: [StepResult], continuation: PlanReport) -> PlanReport {
        ...
        return PlanReport(steps: merged, sessionId: continuation.sessionId, suspendQuestion: continuation.suspendQuestion)
    }
}
