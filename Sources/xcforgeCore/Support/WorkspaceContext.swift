...
public actor SessionState {
    ...
    public func resolveSimulator(_ explicit: String?) async throws -> String {
        if let explicit {
            trackUsage(value: explicit, streak: &simulatorStreak, stored: &simulator, source: &simulatorSource)
            return explicit
        }
    ...
    public func setDefaults(project: String?, scheme: String?, simulator: String?) {
        if let p = project { self.project = p; self.projectSource = .explicit }
        if let s = scheme { self.scheme = s; self.schemeSource = .explicit }
        if let sim = simulator { self.simulator = sim; self.simulatorSource = .explicit }
    ...
    public func showDefaults() -> String {
        ...
        lines.append("  simulator: \(annotated(simulator, source: simulatorSource, nilLabel: "(auto-detect — queries booted sim each call)"))")
        lines.append("  bundle_id: \(bundleId ?? "(from last build)")")
        lines.append("  app_path:  \(appPath ?? "(from last build)")")
        return lines.joined(separator: "\n")
    }

    private func annotated(_ value: String?, source: DefaultsSource, nilLabel: String = "(auto-detect)") -> String {
    ...
    public func profileSwitch(name: String) -> String {
        ...
        if let p = loaded.project { self.project = p; self.projectSource = .persisted }
        if let s = loaded.scheme { self.scheme = s; self.schemeSource = .persisted }
        if let sim = loaded.simulator { self.simulator = sim; self.simulatorSource = .persisted }
        persistCurrentDefaults()
    ...
    private func trackUsage(
        value: String, streak: inout (value: String, count: Int), stored: inout String?, source: inout DefaultsSource
    ) {
    ...
    static func handleProfileSwitch(_ args: [String: Value]?, env: Environment) async -> CallTool.Result {
        switch ToolInput.decode(ProfileNameInput.self, from: args) {
        case .failure(let err): return err
    ...
    static func handleProfileDelete(_ args: [String: Value]?, env: Environment) async -> CallTool.Result {
        switch ToolInput.decode(ProfileNameInput.self, from: args) {
        case .failure(let err): return err
