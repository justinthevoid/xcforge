...
public struct DiagnosisInspectWorkflow: Sendable {
        ...
        do {
            ...
            return DiagnosisInspectResult(
                ...
                failure: WorkflowFailure(
                    ...
                    inferred: InferredFailureConclusion(summary: "An unexpected error occurred during troubleshooting inspection; the underlying issue may be transient or environmental."),
    ...
    static func evidenceCompleteness(for run: WorkflowRunRecord) -> DiagnosisInspectEvidenceCompleteness {
        guard !run.evidence.isEmpty || run.diagnosisSummary != nil || run.testDiagnosisSummary != nil || run.runtimeSummary != nil else {
            return .empty
        }
    ...
    private static func terminalFailure(for run: WorkflowRunRecord) -> WorkflowFailure? {
        ...
        return WorkflowFailure(
            ...
            inferred: InferredFailureConclusion(
                summary: "The failure occurred before or without producing a phase-specific diagnosis summary."
            ),
            recoverability: .retryAfterFix
        )
    }
