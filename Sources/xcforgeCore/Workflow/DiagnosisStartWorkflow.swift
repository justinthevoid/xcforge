...
public struct DiagnosisStartWorkflow: Sendable {
    ...
    typealias SimulatorPreparationResolver = @Sendable (String) async throws -> WorkflowSimulatorPreparation
    ...
    init(
        ...
        validateResolvedScheme: @escaping SchemePreflightValidator = AutoDetect.validateScheme(_:project:),
        validateResolvedSimulator: @escaping SimulatorPreflightValidator = AutoDetect.validateSimulator(_:),
        prepareSimulatorContext: @escaping SimulatorPreparationResolver = AutoDetect.prepareSimulatorContext,
    ...
    public func start(request: DiagnosisStartRequest) async -> DiagnosisStartResult {
        ...
        do {
            ...
            let (appResolution, simulatorPreparation, environmentPreflight) = try await runEnvironmentPreflight(
                ...
                reusedAppContext: contextResolution.reuseAppContext ? reusableRun?.resolvedContext.app : nil,
            ...
            let run = WorkflowRunRecord(
                ...
                actionHistory: [
                    ...
                    WorkflowActionRecord(
                        ...
                        attemptId: attemptId,
                        timestamp: timestamp,
                        detail: "Resolved project: \(resolvedContext.project), scheme: \(resolvedContext.scheme)"
                    )
                ]
            )
            ...
            do {
                ...
                return DiagnosisStartResult(
                    ...
                    failure: WorkflowFailure(
                        ...
                        message: "xcforge validated the local environment but could not persist the run record: \(error)",
                        observed: ObservedFailureEvidence(
                            summary: "xcforge validated the local environment but could not persist the run record: \(error)",
                            detail: String(describing: error)
                        ),
                        inferred: InferredFailureConclusion(
                            summary: "The run record could not be written to disk. A file-system or permissions issue may be preventing persistence."
                        ),
            ...
            return DiagnosisStartResult(
                ...
                failure: WorkflowFailure(
                    ...
                    inferred: InferredFailureConclusion(
                        summary: "An unexpected error occurred during workflow context resolution. The underlying cause may require investigation."
                    ),
    ...
    private func runEnvironmentPreflight(
        ...
        validatedAt: Date
    ) async throws -> (ResolvedContextValue<AppContext>, WorkflowSimulatorPreparation, WorkflowEnvironmentPreflight) {
        ...
        do {
            ...
            checks.append(
                WorkflowEnvironmentCheck(
                    ...
                    field: .simulator,
                    status: .passed,
                    message: "Simulator target prepared for workflow use: \(simulatorPreparation.displayName) (\(simulatorPreparation.selected))."
                )
            )
        } catch {
            throw Self.preflightFailure(
        ...
        do {
            if let reusedAppContext {
                ...
                app = try await appContextResolver(project, scheme, simulatorPreparation.selected, configuration)
            }
            checks.append(
                WorkflowEnvironmentCheck(
                    kind: .appContext,
                    field: .app,
                    status: .passed,
                    message: "App context resolved for scheme '\(scheme)' and configuration '\(configuration)'."
                )
            )
        } catch {
            let classification = Self.classifyAppContextError(error)
            let status: WorkflowEnvironmentPreflightStatus = classification == .unsupportedContext ? .unsupported : .failed
        ...
        if let reusableRun {
            ...
            guard isValid else {
                throw Self.contextResolutionError(
                    ...
                    message: "Reused scheme '\(value)' from run \(reusableRun.runId) is not valid for project \(project).",
                    reusableRun: reusableRun,
                    provenanceFields: currentProvenanceFields,
                    attemptedProvenance: Self.fieldProvenance(
                        field: .scheme,
        ...
        if reuseAppContext, let reusableRun {
            ...
            guard FileManager.default.fileExists(atPath: app.appPath) else {
                throw Self.contextResolutionError(
                    field: .app,
                    classification: .notFound,
                    message: "Reused app context from run \(reusableRun.runId) is no longer available at \(app.appPath).",
                    reusableRun: reusableRun,
                    provenanceFields: currentProvenanceFields,
                    attemptedProvenance: Self.fieldProvenance(
                        field: .app,
            ...
            if let bundle = Bundle(path: app.appPath),
               ...
               bundleId != app.bundleId {
                throw Self.contextResolutionError(
                    field: .app,
                    classification: .unsupportedContext,
                    message: "Reused app context from run \(reusableRun.runId) no longer matches the app bundle at \(app.appPath).",
                    reusableRun: reusableRun,
                    provenanceFields: currentProvenanceFields,
                    attemptedProvenance: Self.fieldProvenance(
                        field: .app,
    ...
    private static func contextResolutionError(
        ...
        let hasReusedField = attemptedProvenance.source == .reusedRun
            || provenanceFields.contains { $0.source == .reusedRun }
        return WorkflowContextResolutionError(
            field: field,
            classification: classification,
            message: message ?? Self.contextualMessage(
                for: field,
    ...
    private static func resolveExplicitValue(
        ...
        guard !trimmed.isEmpty else {
            throw WorkflowContextResolutionError(
                field: field,
                classification: .resolutionFailed,
                message: "Explicit \(field.rawValue.replacingOccurrences(of: "_", with: " ")) override must not be empty.",
                options: []
            )
        }
        return trimmed
    ...
    private static func classifyProjectValidationError(_ error: Error) -> WorkflowFailureClassification {
        let message = "\(error)".lowercased()
        if message.contains("not found") {
            return .notFound
        }
        if message.contains(".xcodeproj") || message.contains(".xcworkspace") || message.contains("unsupported") {
    ...
    private static func classifySimulatorValidationError(_ error: Error) -> WorkflowFailureClassification {
        let message = "\(error)".lowercased()
    ...
    private static func classifySimulatorPreparationError(_ error: Error) -> WorkflowFailureClassification {
        let message = "\(error)".lowercased()
        if message.contains("not available") || message.contains("not found") {
            return .notFound
        }
    ...
    private static func classifyToolingError(_ error: Error) -> WorkflowFailureClassification {
        ...
        if message.contains("unable to find utility") ||
            message.contains("active developer environment") ||
            message.contains("developer directory") ||
