...
public struct DiagnosisVerifyWorkflow: Sendable {
    ...
    typealias ExecuteBuild = @Sendable (ResolvedWorkflowContext) async throws -> TestTools.BuildDiagnosisExecution
    typealias ExecuteTest = @Sendable (ResolvedWorkflowContext) async throws -> TestTools.TestDiagnosisExecution
    typealias ResolveAppContext = @Sendable (String, String, String, String) async throws -> AppContext
    ...
    public func verify(request: DiagnosisVerifyRequest) async -> DiagnosisVerifyResult {
        do {
            ...
            let attemptNumber = (run.backfilledAttemptHistory.map { $0.attempt.attemptNumber }.max() ?? run.attempt.attemptNumber) + 1
            ...
            case .diagnosisStart:
                throw DiagnosisVerifyWorkflowError(
                    ...
                    message: "Run \(run.runId) has not completed a build or test diagnosis yet, so there is no validation path to rerun."
                )
            case .diagnosisRuntime:
                throw DiagnosisVerifyWorkflowError(
                    ...
                    field: .run,
                    classification: .invalidRunState,
                    message: "Run \(run.runId) is in diagnosis_runtime; verification reruns are currently limited to build and test diagnosis phases."
                )
            }
        } catch let error as DiagnosisVerifyWorkflowError {
            ...
            return DiagnosisVerifyResult(
                ...
                failure: WorkflowFailure(
                    ...
                    inferred: InferredFailureConclusion(summary: "An unexpected error occurred during verification; the underlying issue may be transient or environmental."),
    ...
    private func verifyBuild(
        ...
        do {
            ...
            let updatedRun = WorkflowRunRecord(
                ...
                evidence: run.evidence + DiagnosisBuildWorkflow.evidence(
                    for: execution,
                    attempt: attempt,
                    phase: .diagnosisBuild
                ),
            ...
            let updatedRun = WorkflowRunRecord(
                ...
                testDiagnosisSummary: run.testDiagnosisSummary,
                environmentPreflight: run.environmentPreflight,
                evidence: run.evidence + DiagnosisBuildWorkflow.unavailableEvidence(
                    for: attempt,
                    phase: .diagnosisBuild
                ),
                attemptHistory: run.backfilledAttemptHistory + [
            ...
            return persistedVerifyResult(
                ...
                failure: WorkflowFailure(
                    ...
                    inferred: InferredFailureConclusion(summary: "Build execution failed during verification rerun; the build environment or project configuration may need attention."),
        ...
        do {
            ...
            if let blocker = Self.classifyExecutionBlocker(execution) {
                ...
                let updatedRun = WorkflowRunRecord(
                    ...
                    environmentPreflight: run.environmentPreflight,
                    evidence: run.evidence + DiagnosisTestWorkflow.evidence(
                        for: execution,
                        attempt: attempt,
                        phase: .diagnosisTest
                    ),
                ...
                return persistedVerifyResult(
                    ...
                    failure: WorkflowFailure(
                        ...
                        inferred: InferredFailureConclusion(summary: "Test execution was blocked before any test results could be collected; the test environment or configuration may need attention."),
            ...
            let updatedRun = WorkflowRunRecord(
                ...
                environmentPreflight: run.environmentPreflight,
                evidence: run.evidence + DiagnosisTestWorkflow.unavailableEvidence(
                    for: attempt,
                    phase: .diagnosisTest,
            ...
            return persistedVerifyResult(
                ...
                failure: WorkflowFailure(
                    ...
                    observed: ObservedFailureEvidence(summary: classifiedFailure.message),
                    inferred: InferredFailureConclusion(summary: "Test execution failed during verification rerun; the test environment or project configuration may need attention."),
                    recoverability: classifiedFailure.classification.recoverability
                )
            )
        }
        ...
        do {
            ...
            return DiagnosisVerifyResult(
                ...
                failure: WorkflowFailure(
                    ...
                    inferred: InferredFailureConclusion(summary: "The verification completed but its results could not be saved; a filesystem or permissions issue may be preventing persistence."),
                    recoverability: .retryAfterFix
                ),
                persistedRunPath: nil
            )
    ...
    private func resolveContext(
        ...
        let effectiveProject = try Self.resolveOverride(request.project, field: .project) ?? base.project
        let effectiveScheme = try Self.resolveOverride(request.scheme, field: .scheme) ?? base.scheme
        let effectiveSimulator = try Self.resolveOverride(request.simulator, field: .simulator) ?? base.simulator
        let effectiveConfiguration = try Self.resolveOverride(request.configuration, field: .build) ?? base.configuration

        let changed = effectiveProject != base.project
    ...
    private func classifyBuildOutcome(
        ...
        if status == .succeeded {
            return hasSupportingEvidence(evidence, summaryAvailable: rerunSummary != nil) ? .verified : .partial
        }
    ...
    private func hasSupportingEvidence(
        ...
        return evidence.contains { record in
            record.availability == .available && record.kind != .buildSummary && record.kind != .testSummary
        }
    }
    ...
    private static func validate(_ run: WorkflowRunRecord) throws {
        ...
        guard run.phase == .diagnosisBuild || run.phase == .diagnosisTest else {
            throw DiagnosisVerifyWorkflowError(
                ...
                field: .run,
                classification: .invalidRunState,
                message: "Run \(run.runId) is in phase \(run.phase.rawValue); rerun validation requires a persisted build or test diagnosis."
            )
        }
        ...
        guard run.status == .failed || run.status == .partial || run.status == .unsupported else {
            throw DiagnosisVerifyWorkflowError(
                ...
                field: .run,
                classification: .invalidRunState,
                message: "Run \(run.runId) has status \(run.status.rawValue); rerun validation currently requires a failed, partial, or unsupported diagnosis result."
            )
        }
    }
    ...
    private static func sameBuildFailure(
        ...
        guard let lhsObserved = lhs?.observedEvidence,
              let rhsObserved = rhs?.observedEvidence else {
            return false
        }
    ...
    private func persistResolutionFailure(
        ...
        let updatedRun = WorkflowRunRecord(
            ...
            evidence: run.phase == .diagnosisBuild
                ? run.evidence + DiagnosisBuildWorkflow.unavailableEvidence(for: attempt, phase: .diagnosisBuild)
                : run.evidence + DiagnosisTestWorkflow.unavailableEvidence(
                    for: attempt,
                    phase: .diagnosisTest,
