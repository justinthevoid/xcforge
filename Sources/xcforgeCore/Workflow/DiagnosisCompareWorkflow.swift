...
public struct DiagnosisCompareWorkflow: Sendable {
    ...
    public func compare(request: DiagnosisCompareRequest) async -> DiagnosisCompareResult {
        do {
            ...
            let currentSnapshot = run.attemptSnapshot(forAttemptId: run.attempt.attemptId, phase: currentPhase) ?? run.latestSnapshot

            guard let sourceSnapshot = resolveSourceSnapshot(
                for: currentSnapshot,
                in: run
            ) else {
                throw DiagnosisCompareWorkflowError(
            ...
            guard currentAttempt.phase == priorAttempt.phase else {
                throw DiagnosisCompareWorkflowError(
                    field: .run,
                    classification: .invalidRunState,
                    message: "Run \(run.runId) compares phase \(priorAttempt.phase.rawValue) against \(currentAttempt.phase.rawValue); comparison requires a matched build or test rerun."
                )
            }
            ...
            return DiagnosisCompareResult(
                ...
                failure: WorkflowFailure(
                    ...
                    inferred: InferredFailureConclusion(summary: "An unexpected error occurred during comparison; the underlying issue may be transient or environmental."),
    ...
    private func resolveSourceSnapshot(
        ...
        guard let sourceAttemptId = currentSnapshot.attempt.rerunOfAttemptId?.trimmingCharacters(in: .whitespacesAndNewlines),
              !sourceAttemptId.isEmpty else {
            return nil
        }

        guard var sourceSnapshot = run.attemptSnapshot(
        ...
        while let parentAttemptId = sourceSnapshot.attempt.rerunOfAttemptId?.trimmingCharacters(in: .whitespacesAndNewlines),
    ...
    private static func validate(_ run: WorkflowRunRecord) throws {
        ...
        guard run.phase == .diagnosisBuild || run.phase == .diagnosisTest || run.phase == .diagnosisRuntime else {
            throw DiagnosisCompareWorkflowError(
                field: .run,
                classification: .invalidRunState,
                message: "Run \(run.runId) is in phase \(run.phase.rawValue); comparison requires a completed build, test, or runtime diagnosis."
            )
        }
    ...
    ) -> (outcome: DiagnosisCompareOutcome, changedEvidence: [DiagnosisComparisonChange], unchangedBlockers: [String]) {
    ...
    private func compareBuild(
        ...
        let changes = buildComparisonChanges(
    ...
    private func compareTest(
        ...
        let changes = testComparisonChanges(
            priorStatus: priorStatus,
            currentStatus: currentStatus,
            priorSummary: priorSummary,
            currentSummary: currentSummary
    ...
    private func classifyComparison(
        ...
        let improved = buildImprovementDetected(priorSummary: priorSummary, currentSummary: currentSummary)
        let regressed = buildRegressionDetected(priorSummary: priorSummary, currentSummary: currentSummary)
    ...
    private func classifyComparison(
        ...
        let improved = testImprovementDetected(priorSummary: priorSummary, currentSummary: currentSummary)
        let regressed = testRegressionDetected(priorSummary: priorSummary, currentSummary: currentSummary)
    ...
    private func testBlockers(
        ...
        if let priorSummary,
           ...
           priorSummary.observedEvidence.failedTestCount == summary.observedEvidence.failedTestCount {
            blockers.append("Test still reports \(summary.observedEvidence.failedTestCount) failing test(s).")
        }
    ...
    private func evidenceComparisonChanges(
        ...
        let priorAvailable = Set(priorEvidence.filter { $0.availability == .available }.map(evidenceKey))
        let currentAvailable = Set(currentEvidence.filter { $0.availability == .available }.map(evidenceKey))
        let priorUnavailable = Set(priorEvidence.filter { $0.availability == .unavailable }.map(evidenceKey))
        let currentUnavailable = Set(currentEvidence.filter { $0.availability == .unavailable }.map(evidenceKey))
    ...
    private func testImprovementDetected(
        ...
        return current.additionalFailureCount < prior.additionalFailureCount
            || current.totalTestCount > prior.totalTestCount && current.failedTestCount < prior.failedTestCount
