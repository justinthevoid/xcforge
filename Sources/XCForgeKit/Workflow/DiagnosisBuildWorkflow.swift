...
public struct DiagnosisBuildWorkflow: Sendable {
            ...
            do {
                ...
                do {
                    ...
                    throw DiagnosisBuildWorkflowError(
                        field: .workflow,
                        classification: .executionFailed,
                        message: "Build execution failed and xcforge could not persist the failed run state: \(error)",
                        options: [],
                        observed: ObservedFailureEvidence(
    ...
    static func validate(_ run: WorkflowRunRecord) throws {
        ...
        guard run.phase == .diagnosisStart, run.status == .inProgress else {
            throw DiagnosisBuildWorkflowError(
                field: .run,
                classification: .invalidRunState,
                message: "Run \(run.runId) is in phase \(run.phase.rawValue) with status \(run.status.rawValue); build diagnosis currently requires a started diagnosis run.",
                options: [],
                observed: ObservedFailureEvidence(
                    summary: "Run \(run.runId) is in phase \(run.phase.rawValue) with status \(run.status.rawValue); build diagnosis currently requires a started diagnosis run."
                ),
                recoverability: .stop
            )
        }
    ...
    static func buildSummary(from execution: TestTools.BuildDiagnosisExecution) -> BuildDiagnosisSummary {
        ...
        let observed = ObservedBuildEvidence(
            summary: observedSummary(for: execution, primarySignal: primarySignal, uniqueIssueCount: compactIssues.count),
    ...
    static func evidence(
        ...
        evidence.append(
            evidenceRecord(
                ...
                source: "xcodebuild.result_bundle",
                missingDetail: "The build diagnosis reported an xcresult path, but no artifact was present on disk."
            )
        )
        ...
        if let stderrEvidencePath = execution.stderrEvidencePath {
            evidence.append(
                evidenceRecord(
                    ...
                    source: "xcodebuild.stderr",
                    missingDetail: "The build diagnosis reported a stderr artifact path, but no artifact was present on disk."
                )
            )
        } else {
            evidence.append(
    ...
    private static func observedSummary(
        ...
        if let primarySignal, let location = primarySignal.location {
            ...
            if let line = location.line {
                return "Primary signal selected from \(uniqueIssueCount) unique diagnostic(s): \(shortPath):\(line)."
            }
            return "Primary signal selected from \(uniqueIssueCount) unique diagnostic(s): \(shortPath)."
        }

        if execution.errorCount > 0 {
            return "Build failed with \(execution.errorCount) error diagnostic(s); a primary signal was selected from the captured diagnostics."
        }

        return "Build failed, but only limited structured diagnostics were available; inspect the supporting evidence for the full context."
    }

    private static func inferredSummary(primarySignal: TestTools.BuildIssueObservation?) -> String {
        guard let primarySignal else {
            return "The build failed, but a primary structured failure signal could not be inferred from the available diagnostics."
        }
