...
public struct DiagnosisTestWorkflow: Sendable {
        ...
        do {
            ...
            do {
                ...
                let updatedRun = WorkflowRunRecord(
                    ...
                    evidence: run.evidence + Self.unavailableEvidence(
                        for: failedAttempt,
                        phase: .diagnosisTest,
                ...
                do {
                    ...
                    return DiagnosisTestResult(
                        ...
                        failure: WorkflowFailure(
                            field: .workflow,
                            classification: .executionFailed,
                            message: "Test execution failed and xcforge could not persist the failed run state: \(error)",
                            options: [],
                            observed: ObservedFailureEvidence(
                                summary: "Test execution failed and xcforge could not persist the failed run state: \(error)",
                                detail: "Original execution failure: \(classifiedFailure.message)"
                            ),
                            inferred: InferredFailureConclusion(
                                summary: "The test execution failed, and the subsequent attempt to persist the failure state also failed."
                            ),
            ...
            if let blocker = Self.classifyExecutionBlocker(execution) {
                ...
                let updatedRun = WorkflowRunRecord(
                    ...
                    environmentPreflight: run.environmentPreflight,
                    evidence: run.evidence + Self.evidence(
                        for: execution,
                        attempt: updatedAttempt,
                        phase: .diagnosisTest
                    ),
            ...
            return DiagnosisTestResult(
                ...
                failure: WorkflowFailure(
                    ...
                    inferred: error.classification == .executionFailed
                        ? InferredFailureConclusion(summary: "Test execution failed before xcforge could extract test results.")
                        : nil,
    ...
    static func validate(_ run: WorkflowRunRecord) throws {
        ...
        let expectedState = "phase diagnosis_start with status in_progress, or any persisted diagnosis_build phase"
        throw DiagnosisTestWorkflowError(
            status: .failed,
            field: .run,
            classification: .invalidRunState,
            message: "Run \(run.runId) is in phase \(run.phase.rawValue) with status \(run.status.rawValue); test diagnosis currently requires \(expectedState).",
            options: []
        )
    }
    ...
    static func buildSummary(from execution: TestTools.TestDiagnosisExecution) -> TestDiagnosisSummary {
        ...
        if !execution.succeeded && execution.failures.isEmpty {
            let blockerMessage = execution.executionFailureMessage
    ...
    static func evidence(
        ...
        evidence.append(
            evidenceRecord(
                ...
                source: "xcodebuild.result_bundle",
                missingDetail: "The test diagnosis reported an xcresult path, but no artifact was present on disk."
            )
        )
        ...
        if let stderrEvidencePath = execution.stderrEvidencePath {
            evidence.append(
                evidenceRecord(
                    ...
                    source: "xcodebuild.stderr",
                    missingDetail: "The test diagnosis reported a stderr artifact path, but no artifact was present on disk."
                )
            )
        } else {
            evidence.append(
    ...
    static func unavailableEvidence(
        ...
        [
            WorkflowEvidenceRecord(
                ...
                reference: nil,
                source: "xcforge.diagnosis_test.summary",
                detail: "Test execution failed before xcforge could persist a test summary. \(failureMessage)"
            ),
            WorkflowEvidenceRecord(
                kind: .xcresult,
                phase: phase,
    ...
    static func observedSummary(
        ...
        guard let primaryFailure else {
            return "Test run failed, but a primary failing test could not be extracted from the captured diagnostics."
        }

        let headline = primaryFailure.testIdentifier.isEmpty ? primaryFailure.testName : primaryFailure.testIdentifier
        return "Primary failing test selected from \(max(execution.failedTestCount, execution.failures.count)) failing test(s): \(headline)."
    }
    ...
    static func inferredSummary(
        ...
        guard let primaryFailure else {
            return "The test run failed, but a primary failing test could not be inferred from the available diagnostics."
        }
        let headline = primaryFailure.testIdentifier.isEmpty ? primaryFailure.testName : primaryFailure.testIdentifier
        return "The run appears primarily blocked by failing test \(headline): \(primaryFailure.message)"
    }
