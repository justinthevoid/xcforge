...
public struct DiagnosisRuntimeWorkflow: Sendable {
  ...
  typealias CaptureRuntime =
    @Sendable (String, String) async throws -> ConsoleTools.RuntimeSignalCapture
  typealias ResetRuntimeContinuity =
    @Sendable (String, String, WDAClient) async -> SimTools.RuntimeContinuityReset
  typealias CaptureScreenshot =
    @Sendable (String, URL) async -> ScreenshotTools.WorkflowCaptureResult
  ...
  init(
    ...
    resetRuntimeContinuity: @escaping ResetRuntimeContinuity = {
      await ConsoleTools.resetRuntimeCaptureContext(simulatorUDID: $0, bundleId: $1, wdaClient: $2)
    },
  ...
  public func diagnose(request: DiagnosisRuntimeRequest) async -> DiagnosisRuntimeResult {
    ...
    do {
      ...
      guard
        let recoveryPlan = Self.recoveryPlan(
          bundleId: run.resolvedContext.app.bundleId,
          capture: capture,
      ...
      do {
        ...
        persistedFailure = WorkflowFailure(
          ...
          inferred: InferredFailureConclusion(
            summary:
              "Recovery runtime capture failed; the app may have crashed or the simulator environment became unstable."
          ),
      ...
      return DiagnosisRuntimeResult(
        ...
        failure: WorkflowFailure(
          ...
          inferred: InferredFailureConclusion(
            summary:
              "An unexpected error occurred during runtime diagnosis; the workflow could not complete."
          ),
          recoverability: .retryAfterFix
        ),
        persistedRunPath: nil
  ...
  static func validate(_ run: WorkflowRunRecord) throws {
    ...
    case .diagnosisRuntime:
      throw DiagnosisRuntimeWorkflowError(
        ...
        classification: .invalidRunState,
        message:
          "Run \(run.runId) has already completed diagnosis_runtime; repeat runtime or recovery attempts belong to a later workflow story.",
        options: []
      )
    }
  }

  private func persistConsoleArtifact(runId: String, attemptId: String, text: String) throws
    -> String?
  {
  ...
  private static func recoveryPlan(
    ...
    return RuntimeRecoveryPlan(
      ...
      detail:
        "The runtime capture observed launch continuity breaking before the app stayed running."
    )
  }

  private static func recoveryDetail(base: String?, resetWasNeeded: Bool, sessionCleared: Bool)
    -> String
  {
  ...
  private func persistRuntimeFailure(
    ...
    return await persistRuntimeResult(
      ...
      failure: WorkflowFailure(
        ...
        inferred: InferredFailureConclusion(
          summary:
            "Runtime execution failed before xcforge could complete the capture; the simulator or app environment may need attention."
        ),
        recoverability: .retryAfterFix
      )
    )
  ...
  static func runtimeSummary(
    ...
    let observed = ObservedRuntimeEvidence(
      summary: observedSummary(
        bundleId: bundleId, capture: capture, launchDetected: launchDetected),
        ...
        evidence.append(
          WorkflowEvidenceRecord(
            ...
            detail: screenshotCapture?.detail
              ?? "Runtime inspection did not capture a screenshot for this attempt."
          )
        )
      }
    }
    ...
    if requestedScreenshot {
      evidence.append(
        WorkflowEvidenceRecord(
          ...
          detail:
            "Requested screenshot capture could not run because runtime execution failed before the visual-capture step completed."
        )
      )
    }
    return evidence
  ...
  private static func classifyOutcome(
    ...
    let runtimeSignalsCaptured =
      capture.stdout.contains(where: hasContent)
        ...
        return (
          ...
          screenshotFailure
            ?? WorkflowFailure(
              ...
              classification: .executionFailed,
              message:
                "The app launched for runtime inspection but did not remain running for the full capture window.",
              observed: ObservedFailureEvidence(
                summary:
                  "The app launched for runtime inspection but did not remain running for the full capture window."
              ),
              inferred: InferredFailureConclusion(
                summary:
                  "The app may have crashed or been terminated shortly after launch; console output may contain crash signals."
              ),
              recoverability: .retryAfterFix
            )
        )
    ...
    return (
      ...
      WorkflowFailure(
        ...
        inferred: InferredFailureConclusion(
          summary:
            "No reliable launch success signal was captured; the app may have failed to install, launch, or the simulator environment may be misconfigured."
        ),
        recoverability: .retryAfterFix
      )
    )
  ...
  private static func screenshotFailure(
    ...
    return WorkflowFailure(
      ...
      message: capture.detail
        ?? "Requested screenshot capture was unavailable for this runtime attempt.",
      observed: ObservedFailureEvidence(
  ...
  private static func choosePrimarySignal(from capture: ConsoleTools.RuntimeSignalCapture)
    -> RuntimeSignalSummary?
  {
    if let stderr = capture.stderr.first(where: {
      !$0.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty
    }) {
      return RuntimeSignalSummary(
        stream: .stderr, message: stderr, source: "simctl.launch_console.stderr")
    }
    if let stdout = capture.stdout.first(where: {
      !$0.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty
    }) {
      return RuntimeSignalSummary(
        stream: .stdout, message: stdout, source: "simctl.launch_console.stdout")
    }
    return nil
  }
  ...
  private static func inferredSummary(
    ...
    return
      "No reliable launch success signal was captured for \(bundleId); inspect the runtime evidence for environment or app-level blockers."
  }

  private static func chooseFailureMessage(from capture: ConsoleTools.RuntimeSignalCapture)
    -> String
  {
    if let stderr = capture.stderr.first(where: {
      !$0.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty
