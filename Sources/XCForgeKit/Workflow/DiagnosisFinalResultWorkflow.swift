...
public struct DiagnosisFinalResultWorkflow: Sendable {
  ...
  public func assemble(request: DiagnosisFinalResultRequest) async -> DiagnosisFinalResult {
    do {
      ...
      let currentSnapshot =
        run.attemptSnapshot(forAttemptId: run.attempt.attemptId, phase: run.phase)
        ?? run.latestSnapshot
      ...
      let compareWorkflow = DiagnosisCompareWorkflow(
        resolver: RunResolver(
          ...
          loadRun: resolver.loadRun,
          loadLatestActiveRun: resolver.loadLatestActiveRun,
          loadLatestRun: resolver.loadLatestRun
        ),
        runPath: runPath
      )
      let compareResult = await compareWorkflow.compare(
        request: DiagnosisCompareRequest(runId: run.runId))
      ...
      return DiagnosisFinalResult(
        ...
        followOnAction: WorkflowFollowOnAction(
          ...
          rationale: sanitizeRationale(
            "Workflow failed at \(failure.field.rawValue): \(failure.message)"),
      ...
      let failure = WorkflowFailure(
        ...
        inferred: InferredFailureConclusion(
          summary:
            "An unexpected error occurred while assembling final results; the underlying issue may be transient or environmental."
        ),
  ...
  private static func mapResolutionFailure(_ failure: RunResolutionFailure) -> Error {
    switch failure {
    case .emptyRunId:
      return DiagnosisFinalResultWorkflowError(
    ...
    case .runStillInProgress(let runId):
      return DiagnosisFinalResultWorkflowError(
        ...
        message:
          "Run \(runId) is still in progress; final results require a completed diagnosis.")
  ...
  private static func validate(_ run: WorkflowRunRecord) throws {
    ...
    guard
      ...
      throw DiagnosisFinalResultWorkflowError(
        ...
        classification: .invalidRunState,
        message:
          "Run \(run.runId) is in phase \(run.phase.rawValue); final results require a completed build, test, or runtime diagnosis."
      )
    }
    ...
    guard run.status != .inProgress else {
      throw DiagnosisFinalResultWorkflowError(
        ...
        classification: .invalidRunState,
        message:
          "Run \(run.runId) is still in progress; final results require a completed diagnosis."
      )
    }
  }
  ...
  private func comparisonContext(from snapshot: WorkflowAttemptSnapshot) -> (
    sourceAttemptId: String?, note: String?
  ) {
...
extension DiagnosisFinalResult {
  ...
  private static func deriveFollowOnAction(for result: DiagnosisFinalResult)
    -> WorkflowFollowOnAction?
  {
    ...
    if isBlocked {
      ...
      return WorkflowFollowOnAction(
        ...
        rationale:
          "Recovery did not resume after \(count == 1 ? "1 recovery action" : "\(count) recovery actions").",
        confidence: .evidenceSupported
      )
    }
    ...
    if let attempt = result.currentAttempt {
      if let buildSummary = attempt.diagnosisSummary {
        ...
        return WorkflowFollowOnAction(
          ...
          rationale:
            "Build diagnosis identified \(buildSummary.observedEvidence.errorCount) error(s).\(detail)",
          confidence: .evidenceSupported
        )
      }
      if let testSummary = attempt.testDiagnosisSummary {
        let detail =
          testSummary.observedEvidence.primaryFailure.map {
            " Primary failure: \($0.testIdentifier)"
          } ?? ""
        return WorkflowFollowOnAction(
          action: "Review the failing tests and apply a fix, then rerun validation.",
          rationale:
            "\(testSummary.observedEvidence.failedTestCount) of \(testSummary.observedEvidence.totalTestCount) tests failed.\(detail)",
          confidence: .evidenceSupported
        )
      }
      if let runtimeSummary = attempt.runtimeSummary {
        let detail =
          runtimeSummary.observedEvidence.primarySignal.map {
            " Primary signal: \($0.message)"
          } ?? ""
        return WorkflowFollowOnAction(
          action: "Review the runtime failure and captured evidence, then rerun validation.",
          rationale:
            "Runtime diagnosis failed — app \(runtimeSummary.observedEvidence.appRunning ? "was running" : "did not stay running").\(detail)",
          confidence: .evidenceSupported
        )
      }
    }
  ...
  private static func deriveForPartial(_ result: DiagnosisFinalResult) -> WorkflowFollowOnAction {
    if let attempt = result.currentAttempt, !attempt.unavailableEvidence.isEmpty {
      return WorkflowFollowOnAction(
        ...
        rationale:
          "\(attempt.unavailableEvidence.count) expected evidence artifact(s) are unavailable.",
        confidence: .evidenceSupported
      )
    }
