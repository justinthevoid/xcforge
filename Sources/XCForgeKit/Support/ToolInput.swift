...
private struct ValueArgumentDecoder: Decoder {
    ...
    func unkeyedContainer() throws -> UnkeyedDecodingContainer {
        throw DecodingError.typeMismatch([Value].self, .init(codingPath: codingPath, debugDescription: "Expected keyed container"))
    }
...
private struct ValueKeyedContainer<Key: CodingKey>: KeyedDecodingContainerProtocol {
    ...
    func decodeIfPresent<T: Decodable>(_ type: T.Type, forKey key: Key) throws -> T? {
        ...
        if type == Bool.self || type == Optional<Bool>.self { return try decode(Bool.self, forKey: key) as? T }
        if type == String.self || type == Optional<String>.self { return try decode(String.self, forKey: key) as? T }
        if type == Int.self || type == Optional<Int>.self { return try decode(Int.self, forKey: key) as? T }
        if type == Double.self || type == Optional<Double>.self { return try decode(Double.self, forKey: key) as? T }
    ...
    func nestedContainer<NestedKey: CodingKey>(keyedBy type: NestedKey.Type, forKey key: Key) throws -> KeyedDecodingContainer<NestedKey> {
        ...
        return KeyedDecodingContainer(ValueKeyedContainer<NestedKey>(values: dict, codingPath: codingPath + [key]))
    }
    ...
    private func missing(_ key: Key) -> DecodingError {
        .keyNotFound(key, .init(codingPath: codingPath, debugDescription: "Key '\(key.stringValue)' not found"))
    }

    private func mismatch(_ type: Any.Type, _ key: Key) -> DecodingError {
        .typeMismatch(type, .init(codingPath: codingPath + [key], debugDescription: "Type mismatch for '\(key.stringValue)'"))
    }
}
...
private struct ValueArrayDecoder: Decoder {
    ...
    func container<Key: CodingKey>(keyedBy type: Key.Type) throws -> KeyedDecodingContainer<Key> {
        throw DecodingError.typeMismatch([String: Value].self, .init(codingPath: codingPath, debugDescription: "Expected array"))
    }
    ...
    func singleValueContainer() throws -> SingleValueDecodingContainer {
        throw DecodingError.typeMismatch(Value.self, .init(codingPath: codingPath, debugDescription: "Expected array"))
    }
}
...
private struct ValueArrayDecodingContainer: UnkeyedDecodingContainer {
    ...
    private struct IndexKey: CodingKey {
        ...
        var intValue: Int?
        init(_ index: Int) { self.stringValue = "\(index)"; self.intValue = index }
    ...
    mutating func decodeNil() throws -> Bool {
        ...
        if values[currentIndex] == .null { currentIndex += 1; return true }
        return false
    }
    ...
    mutating func decode(_ type: Int.Type) throws -> Int {
        ...
        case .int(let n): return n
        case .double(let n): return Int(n)
        case .string(let s): if let d = Double(s) { return Int(d) }; throw typeMismatch(type)
        default: throw typeMismatch(type)
        }
    }
    ...
    mutating func decode(_ type: Double.Type) throws -> Double {
        ...
        case .double(let n): return n
        case .int(let n): return Double(n)
        case .string(let s): if let d = Double(s) { return d }; throw typeMismatch(type)
        default: throw typeMismatch(type)
        }
    }
    ...
    mutating func nestedContainer<NestedKey: CodingKey>(keyedBy type: NestedKey.Type) throws -> KeyedDecodingContainer<NestedKey> {
        ...
        return KeyedDecodingContainer(ValueKeyedContainer<NestedKey>(values: dict, codingPath: codingPath))
    }
    ...
    private func typeMismatch(_ type: Any.Type) -> DecodingError {
        .typeMismatch(type, .init(codingPath: codingPath + [IndexKey(currentIndex - 1)], debugDescription: "Type mismatch at index \(currentIndex - 1)"))
    }
}
