...
public struct RunStore: Sendable {
    ...
    public init(baseDirectory: URL? = nil) {
        ...
        if let override = ProcessInfo.processInfo.environment["XCFORGE_RUN_STORE_DIR"], !override.isEmpty {
    ...
    public func latestTerminalDiagnosisRun() throws -> WorkflowRunRecord? {
        try listDiagnosisRuns().first {
            $0.status != .inProgress
                && ($0.phase == .diagnosisBuild || $0.phase == .diagnosisTest || $0.phase == .diagnosisRuntime)
        }
    }
...
private enum RunStoreError: LocalizedError {
    ...
    var errorDescription: String? {
        switch self {
        case let .invalidReusableDiagnosisRun(runId, workflow):
            return "Run \(runId) is a \(workflow) workflow record and cannot seed diagnosis context reuse."
        }
    }
}
