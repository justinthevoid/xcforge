...
public final class PlanExecutor: Sendable {
    ...
    public init(session: SessionState, wdaClient: WDAClient, errorStrategy: ErrorStrategy = .abortWithScreenshot, timeoutSeconds: Double = 120) {
    ...
    public func restore(priorResults: [StepResult], savedBindings: [String: VariableStore.ElementBinding], startTime: CFAbsoluteTime) {
    ...
    private func runStep(_ step: PlanStep, index: Int) async -> StepOutcome {
        do {
            ...
            case .find(let target, let using, let bindAs):
                ...
                return .result(.passed, detail: "Found '\(target)'\(bindAs.map { " → \($0)" } ?? "")", screenshot: nil)
            ...
            case .findAll(let targets, let using):
                ...
                return .result(.passed, detail: "Found \(details.count) elements: \(details.joined(separator: ", "))", screenshot: nil)
            ...
            case .click(let target):
                ...
                if IndigoHIDClient.isAvailable {
                    do {
                        ...
                        return .result(.passed, detail: "Clicked \(targetDescription(target)) (indigo)", screenshot: nil)
                    } catch {
            ...
            case .doubleTap(let target):
                ...
                if IndigoHIDClient.isAvailable {
                    do {
                        ...
                        return .result(.passed, detail: "Double-tapped \(targetDescription(target)) (indigo)", screenshot: nil)
                    } catch {
                ...
                return .result(.passed, detail: "Double-tapped \(targetDescription(target))", screenshot: nil)
            ...
            case .longPress(let target, let durationMs):
                ...
                if IndigoHIDClient.isAvailable {
                    do {
                        ...
                        return .result(.passed, detail: "Long-pressed \(targetDescription(target)) (indigo)", screenshot: nil)
                    } catch {
                ...
                return .result(.passed, detail: "Long-pressed \(targetDescription(target))", screenshot: nil)

            case .swipe(let direction, let target, let durationMs):
                let dur = durationMs ?? 300
                let size = try await wdaClient.getWindowSize()
                let (sx, sy, ex, ey) = swipeCoordinates(direction: direction, width: size.width, height: size.height)
            ...
            case .typeText(let text, let target):
                ...
                return .result(.passed, detail: "Typed '\(text.prefix(20))\(text.count > 20 ? "..." : "")'", screenshot: nil)
            ...
            case .waitFor(let text, let timeout, let condition):
                ...
                let predicate = "label == '\(escaped)' OR identifier == '\(escaped)' OR value == '\(escaped)'"
                var found = false
                ...
                case .appears:
                    ...
                    return .result(.failed, detail: "'\(text)' did not appear within \(maxWait)s", screenshot: nil)
                case .disappears:
                    if !found {
                        return .result(.passed, detail: "'\(text)' disappeared", screenshot: nil)
                    }
                    return .result(.failed, detail: "'\(text)' still present after \(maxWait)s", screenshot: nil)
                }
            ...
            case .ifElementExists(let using, let value, let thenSteps):
                do {
                    ...
                    return .result(.passed, detail: "Conditional branch executed (\(thenSteps.count) sub-steps)", screenshot: nil)
                } catch {
                    return .result(.skipped, detail: "Element not found, skipped conditional branch", screenshot: nil)
                }
            ...
            case .handleUnexpected(let description):
                let ss = await captureScreenshot()
                return .suspend(question: "Unexpected state: \(description). How should I proceed?", screenshot: ss)
            }
        } catch {
    ...
    private func captureScreenshot() async -> String? {
        if #available(macOS 14.0, *) {
            do {
                ...
                let result = try await FramebufferCapture.captureInline(simulator: sim, format: "jpeg", quality: 0.7)
    ...
    private func swipeCoordinates(direction: String, width: Double, height: Double) -> (Double, Double, Double, Double) {
        let cx = width / 2
        let cy = height / 2
