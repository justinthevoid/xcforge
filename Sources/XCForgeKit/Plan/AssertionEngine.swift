...
public enum VerifyEngine {
  public static func verify(_ condition: VerifyCondition, wdaClient: WDAClient) async
    -> VerifyResult
  {
    switch condition {
    ...
    case .elementCount(let using, let value, let op, let expected):
      return await verifyElementCount(
        using: using, value: value, op: op, expected: expected, wdaClient: wdaClient)
    }
  }

  // MARK: - screenContains

  private static func verifyScreenContains(_ text: String, wdaClient: WDAClient) async
    -> VerifyResult
  {
    do {
  ...
  private static func verifyElementLabel(
    id: String, op: VerifyCondition.LabelOp, expected: String, wdaClient: WDAClient
  ) async -> VerifyResult {
    do {
      ...
      case .equals:
        ...
        return VerifyResult(
          passed: false, detail: "Element '\(id)' label is '\(actual)', expected '\(expected)'")
      case .contains:
        if actual.contains(expected) {
          return VerifyResult(passed: true, detail: "Element '\(id)' label contains '\(expected)'")
        }
        return VerifyResult(
          passed: false, detail: "Element '\(id)' label '\(actual)' does not contain '\(expected)'")
      }
    } catch {
  ...
  private static func verifyElementExists(using: String, value: String, wdaClient: WDAClient) async
    -> VerifyResult
  {
    do {
  ...
  private static func verifyElementNotExists(using: String, value: String, wdaClient: WDAClient)
    async -> VerifyResult
  {
    do {
      _ = try await wdaClient.findElement(using: using, value: value)
  ...
  private static func verifyElementCount(
    using: String, value: String, op: VerifyCondition.CountOp, expected: Int, wdaClient: WDAClient
  ) async -> VerifyResult {
    do {
