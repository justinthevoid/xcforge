...
enum CoreSimCapture {
  ...
  nonisolated(unsafe) private static let coreSimHandle: UnsafeMutableRawPointer? = {
    ...
    dlopen("/Library/Developer/PrivateFrameworks/CoreSimulator.framework/CoreSimulator", RTLD_NOW)
      ?? dlopen(
        "\(developerDir)/Library/PrivateFrameworks/CoreSimulator.framework/CoreSimulator", RTLD_NOW)
  }()
  ...
  private static func createCGImageDirect(from surface: IOSurfaceRef) -> CGImage? {
    ...
    let expectedBGRA: OSType = 0x4247_5241

    guard width > 0, height > 0, bytesPerElement == 4, pixelFormat == expectedBGRA else {
      Log.warn(
        "CoreSimCapture: unexpected surface format — \(width)×\(height), \(bytesPerElement) bpe, pixelFormat=\(pixelFormat)"
      )
      return nil
    }
    ...
    guard
      let data = CFDataCreate(
        nil, baseAddress.assumingMemoryBound(to: UInt8.self), bytesPerRow * height)
    else {
      return nil
    }
