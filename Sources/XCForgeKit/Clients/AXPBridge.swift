...
public actor AXPBridge {
  ...
  nonisolated static let isAvailable: Bool = {
    ...
    if !trusted {
      Log.warn(
        "AXPBridge unavailable — process not trusted for accessibility. getSource will use WDA.")
    }
    return trusted
  }()
  ...
  func performClick(handle: AXPHandle) async throws {
    ...
    try await runOffActor(
      timeout: timeout, timeoutError: { .actionFailed("AXPress timed out after \(Int(timeout))s") }
    ) {
  ...
  func getText(handle: AXPHandle) async throws -> String {
    ...
    return try await runOffActor(
      timeout: timeout, timeoutError: { .actionFailed("getText timed out after \(Int(timeout))s") }
    ) {
  ...
  private func snapshotTree(udid: String? = nil) async throws -> [AXElement] {
    ...
    let elements: [AXElement] = try await runOffActor(
      timeout: timeout,
      timeoutError: { .treeBuildFailed("Tree traversal timed out after \(Int(timeout))s") }
    ) {
  ...
  private static func traverse(
    ...
    for child in children {
      traverse(
        element: child, into: &elements, depth: depth + 1, maxDepth: maxDepth,
