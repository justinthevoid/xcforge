...
actor IndigoHIDClient {
  ...
  private func resolvePort(simulator: String) async throws -> mach_port_t {
    ...
    let port = try await withCheckedThrowingContinuation {
      (continuation: CheckedContinuation<mach_port_t, Error>) in
    ...
    for (_, devices) in runtimes {
      for device in devices {
        guard let devUDID = device["udid"] as? String, devUDID == udid,
          let deviceTypeId = device["deviceTypeIdentifier"] as? String
        else { continue }
        ...
        if let dynamic = try? await Self.dynamicScreenDimensions(for: deviceTypeId) {
          ...
          Log.warn(
            "IndigoHID: dynamic screen lookup failed for \(deviceTypeId), using hardcoded fallback")
  ...
  static func screenDimensions(for deviceType: String) -> (
    ...
    let model = deviceType.replacingOccurrences(
      of: "com.apple.CoreSimulator.SimDeviceType.", with: "")
  ...
  static func dynamicScreenDimensions(for deviceType: String) async throws -> (
    width: Double, height: Double, scale: Double
  ) {
  ...
  private static func sendTouch(
    port: mach_port_t, xRatio: Float, yRatio: Float, direction: TouchDirection
  ) throws {
