...
public enum ToolRegistry {
    ...
    private static let toolGroupsTool = Tool(
        name: "tool_groups",
        description: "List, enable, or disable tool groups at runtime to reduce MCP tool surface. Changes are runtime-only (not persisted).",
        inputSchema: .object([
            "type": .string("object"),
            "properties": .object([
                "list": .object([
    ...
    private static func handleToolGroups(_ args: [String: Value]?) -> CallTool.Result {
        ...
        case .success(let input):
            ...
            if let toEnable = input.enable {
                ...
                if !unknown.isEmpty {
                    return .fail("Unknown group(s): \(unknown.joined(separator: ", ")). Valid: \(allGroups.joined(separator: ", "))")
                }
            ...
            if let toDisable = input.disable {
                ...
                if !blocked.isEmpty {
                    return .fail("Cannot disable protected group(s): \(blocked.joined(separator: ", ")). These provide essential session management tools.")
                }
