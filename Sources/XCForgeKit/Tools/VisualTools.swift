...
public enum VisualTools {
  ...
  public static let tools: [Tool] = [
    Tool(
      ...
      inputSchema: .object([
        ...
        "properties": .object([
          "name": .object([
            "type": .string("string"),
            "description": .string("Baseline name (e.g. 'login-screen'). Used as filename."),
          ]),
          "simulator": .object([
            "type": .string("string"),
            "description": .string(
          ...
          "baseline_dir": .object([
            "type": .string("string"),
            "description": .string("Directory to store baselines. Default: visual-baselines/"),
          ]),
        ]),
    ...
    Tool(
      name: "compare_visual",
      description:
        "Take a screenshot and compare it pixel-by-pixel against a saved baseline. Returns diff percentage and a diff image path.",
      inputSchema: .object([
        "type": .string("object"),
        "properties": .object([
          "name": .object([
            "type": .string("string"), "description": .string("Baseline name to compare against."),
          ]),
          "simulator": .object([
            "type": .string("string"),
            "description": .string(
          ...
          "threshold": .object([
            "type": .string("number"),
            "description": .string("Max allowed diff percentage (0-100). Default: 0.5"),
          ]),
          "baseline_dir": .object([
            "type": .string("string"),
            "description": .string(
              "Directory where baselines are stored. Default: visual-baselines/"),
          ]),
        ]),
        "required": .array([.string("name")]),
      ])
  ...
  static func saveVisualBaseline(_ args: [String: Value]?, env: Environment) async
    -> CallTool.Result
  {
    ...
    -> CallTool.Result
  {
    ...
    let baselinePath = "\(baselineDir)/\(sanitize(input.name)).png"
    let currentPath = "/tmp/xcf-visual-current-\(runId).png"
    let diffPath = "/tmp/xcf-visual-diff-\(sanitize(input.name))-\(runId).png"
    ...
    do {
      ...
      if comparison.sizeMismatch {
        lines.append(
          "WARNING: Size mismatch — baseline: \(baselineImage.width)x\(baselineImage.height), current: \(currentImage.width)x\(currentImage.height)"
        )
      }
  ...
  public static func captureCGImage(simulator: String, env: Environment = .live) async throws
    -> CGImage
  {
    if CoreSimCapture.isAvailable {
      do {
  ...
  private static func simctlCapture(sim: String, tmpPrefix: String, env: Environment) async throws
    -> CGImage
  {
    let tmpPath = "/tmp/xcf-\(tmpPrefix)-\(UUID().uuidString).png"
  ...
  public static func loadCGImage(path: String) -> CGImage? {
    ...
    return CGImage(
      pngDataProviderSource: dataProvider, decode: nil, shouldInterpolate: false,
  ...
  public static func pixelCompare(baseline: CGImage, current: CGImage) -> ComparisonResult {
    ...
    guard
      let baseCtx = CGContext(
        data: &baselinePixels, width: w, height: h,
        bitsPerComponent: 8, bytesPerRow: bytesPerRow,
        space: colorSpace, bitmapInfo: bitmapInfo.rawValue),
      let currCtx = CGContext(
        data: &currentPixels, width: w, height: h,
        bitsPerComponent: 8, bytesPerRow: bytesPerRow,
        space: colorSpace, bitmapInfo: bitmapInfo.rawValue)
    else {
      return ComparisonResult(
        diffPercent: 100, changedPixels: totalPixels,
    ...
    return ComparisonResult(
      diffPercent: diffPercent, changedPixels: changedPixels,
