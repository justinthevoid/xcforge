...
public actor AppConsole {
  ...
  public func launch(
    simulator: String, bundleId: String, args: [String] = [], env: [String: String] = [:]
  ) throws -> String {
    ...
    var simctlArgs = [
      "simctl", "launch", "--console", "--terminate-running-process", simulator, bundleId,
    ]
    simctlArgs += args
...
enum ConsoleTools {
    ...
    Tool(
      ...
      inputSchema: .object([
        ...
        "properties": .object([
          ...
          "args": .object([
            "type": .string("string"),
            "description": .string("Space-separated launch arguments for the app"),
          ]),
        ]),
      ])
    ),
    Tool(
      ...
      inputSchema: .object([
        ...
        "properties": .object([
          ...
          "stream": .object([
            "type": .string("string"),
            "description": .string("Which stream: stdout, stderr, or both. Default: both"),
          ]),
        ]),
      ])
    ),
    ...
    case .success(let input):
      ...
      do {
        ...
        let msg = try await AppConsole.shared.launch(
          simulator: udid, bundleId: bundleId, args: launchArgs)
        return .ok(msg)
      } catch {
  ...
  static func captureRuntimeSignals(
    ...
    let relaunchedApp = try await SimTools.terminateAppIfRunning(
    ...
    let appRunning =
      (try? await SimTools.terminateAppIfRunning(
        simulatorUDID: simulatorUDID, bundleId: bundleId, env: env)) ?? false
    await AppConsole.shared.stop()

    let combinedText =
      (output.stdout.map { "[stdout] \($0)" }
