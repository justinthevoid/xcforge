...
public enum TestTools {
        ...
        Tool(
            ...
            inputSchema: .object([
                ...
                "properties": .object([
                    ...
                    "testplan": .object(["type": .string("string"), "description": .string("Test plan name. Required when filtering Swift Testing @Test suites (they are not discoverable by -only-testing without a testplan). Optional for XCTest.")]),
                    "filter": .object(["type": .string("string"), "description": .string("Test filter. Accepts: 'testMethod', 'TestClass/testMethod', or full 'TestTarget/TestClass/testMethod'. Target prefix auto-resolution works for XCTest only. Swift Testing @Test suites require the full Target/Suite path AND a testplan to be discoverable.")]),
        ...
        Tool(
            ...
            inputSchema: .object([
                ...
                "properties": .object([
                    ...
                    "scheme": .object(["type": .string("string"), "description": .string("Xcode scheme name. Auto-detected if omitted.")]),
                    "simulator": .object(["type": .string("string"), "description": .string("Simulator name or UDID. Auto-detected if omitted.")]),
                    "filter": .object(["type": .string("string"), "description": .string("Substring filter on test identifiers. Returns only tests whose Target/Class/method contains this string. Use to verify filter format before running test_sim.")]),
                ]),
            ])
        ),
    ]
    ...
    static func resolveFilter(
        ...
        Log.warn("Multiple test targets found: \(targets.joined(separator: ", ")). Cannot auto-resolve filter '\(trimmedFilter)'. Prefix with target name, e.g. '\(targets[0])/\(trimmedFilter)'")
        return trimmedFilter
    }
    ...
    static func zeroMatchHint(
        filter: String, project: String?, scheme: String?, simulator: String?, testplan: String? = nil, env: Environment
    ) async -> String {
        ...
        guard let listResult = try? await executeListTests(
    ...
    private static func exportFailureAttachments(_ xcresultPath: String, env: Environment) async -> [(test: String, path: String)] {
        ...
        guard let manifestResult = try? await env.shell.run("/bin/cat", arguments: ["\(outputDir)/manifest.json"], timeout: 5),
    ...
    private static func extractFailedTestConsole(_ xcresultPath: String, env: Environment) async -> [String: String] {
    ...
    public static func executeTest(
        ...
        if !buildResult.succeeded && failures.isEmpty && parsedSummary == nil {
            ...
            if errorLines.isEmpty {
                ...
                let tail = String(buildResult.stderr.suffix(2000)).trimmingCharacters(in: .whitespacesAndNewlines)
                if !tail.isEmpty {
                    failures = [TestFailureObservation(
                        testName: "xcodebuild",
        ...
        let totalTestCount = parsedSummary?.totalTestCount ?? max(failures.count, buildResult.succeeded ? 0 : 1)
        let failedTestCount = parsedSummary?.failedTestCount ?? failures.count
        let passedTestCount = parsedSummary?.passedTestCount ?? 0
        let skippedTestCount = parsedSummary?.skippedTestCount ?? 0
        let expectedFailureCount = parsedSummary?.expectedFailureCount ?? 0
    ...
    public static func extractFailures(
        ...
        let consoleByTest = (includeConsole && !failures.isEmpty) ? await extractFailedTestConsole(resolvedPath, env: env) : [:]
    ...
    public static func extractCoverage(
        ...
        guard let coverageJSON = await parseCoverage(resolvedPath, env: env),
              ...
              let json = try? JSONSerialization.jsonObject(with: data) as? [String: Any] else {
            throw CoverageError("Failed to parse coverage from \(resolvedPath). Was coverage enabled during the test run?")
        }
        ...
        return CoverageResult(overallCoverage: overallCoverage, targets: targets, xcresultPath: resolvedPath)
    }
    ...
    public static func extractFileCoverage(
        ...
        guard result.succeeded, !result.stdout.isEmpty else {
            throw CoverageError("No coverage data for '\(file)'. File not in coverage report or coverage not enabled.\n\(result.stderr)")
        }
        ...
        guard let fileObj = matched.first else {
            ...
            throw CoverageError("'\(file)' not found in coverage. Available: \(available.joined(separator: ", "))")
        }
        ...
        if let versionLine = xcodeVersionResult.stdout.split(separator: "\n").first,
           ...
           let majorVersion = Int(versionString.split(separator: ".").first ?? "") {
            guard majorVersion >= 16 else {
                throw TestDiscoveryError("-enumerate-tests requires Xcode 16 or later (found Xcode \(majorVersion))")
            }
        } else {
    ...
    static func testSim(_ args: [String: Value]?, env: Environment) async -> CallTool.Result {
        ...
        case .success(let input):
            ...
            if testplan != nil && input.filter != nil {
                preamble += "Note: filter and testplan are both set. -only-testing overrides the testplan's test selection. Tests not matching the filter will be skipped regardless of testplan.\n"
            }
            ...
            do {
                ...
                if buildResult.succeeded {
                    var msg = preamble + "Tests passed in \(elapsed)s (xcresult parse failed)\nxcresult: \(path)"
    ...
    static func testFailures(_ args: [String: Value]?, env: Environment) async -> CallTool.Result {
        ...
        case .success(let input):
            ...
            let consoleByTest = includeConsole ? await extractFailedTestConsole(xcresultPath, env: env) : [:]
    ...
    private static func fileCoverage(file: String, xcresultPath: String, env: Environment) async -> CallTool.Result {
    ...
    static func buildAndTest(_ args: [String: Value]?, env: Environment) async -> CallTool.Result {
        ...
        case .success(let input):
            ...
            do {
                ...
                return formatBuildAndTest(result, testplan: input.testplan, filter: input.filter, suffix: zeroHint)
            } catch {
    ...
    static func formatBuildAndTest(
        ...
        if result.buildSucceeded && result.testResult != nil {
            ...
            if testplan != nil && filter != nil {
                lines.append("Note: filter and testplan are both set. -only-testing overrides the testplan's test selection.")
            }
        }
        ...
        if let test = result.testResult {
            ...
            lines.append("  Total: \(test.totalTestCount)  Passed: \(test.passedTestCount)  Failed: \(test.failedTestCount)  Skipped: \(test.skippedTestCount)")
        ...
        return .ok("Build succeeded in \(result.buildElapsed)s, but test result is unavailable." + suffix)
    }
    ...
    static func formatListTests(_ result: ListTestsResult, filter: String? = nil) -> CallTool.Result {
        ...
        if let trimmedFilter, !trimmedFilter.isEmpty {
            ...
            if tests.isEmpty {
                ...
                let suggestions = classNames.filter { $0.lowercased().contains(lowered) }
                    .sorted().prefix(5)
                var msg = "0 tests matched filter \"\(trimmedFilter)\" (out of \(result.testCount) total tests)"
                if !suggestions.isEmpty {
            ...
            filterNote = "Showing \(tests.count) of \(result.testCount) tests matching \"\(trimmedFilter)\""
        } else {
            tests = result.tests
            filterNote = nil
        }
        ...
        if let note = filterNote {
            ...
            lines.append("Found \(result.testCount) tests in \(result.targetCount) target(s), \(result.classCount) class(es)")
        }
        lines.append("")
        ...
        lines.append("")
        lines.append("Note: Swift Testing @Test suites require the full Target/Suite path and a testplan.")
    ...
    private static func formatTestFailures(
        ...
        let failures = observedFailures.map { failure -> String in
            ...
            let funcName = failure.testIdentifier.split(separator: "/").last.map(String.init) ?? failure.testIdentifier
    ...
    private static func parseTestSummary(_ json: [String: Any]) -> ParsedTestSummary {
        let failures = ((json["testFailures"] as? [[String: Any]]) ?? []).map { failure in
            TestFailureObservation(
                ...
                testIdentifier: (failure["testIdentifierString"] as? String) ?? (failure["testName"] as? String) ?? "?",
                message: (failure["failureText"] as? String) ?? "Test failed without a captured failure message.",
    ...
    private static func formatBuildDiagnosis(_ execution: BuildDiagnosisExecution) -> CallTool.Result {
        ...
        if observed.errorCount > 0 || observed.warningCount > 0 || observed.analyzerWarningCount > 0 {
            ...
            if observed.analyzerWarningCount > 0 { parts.append("\(observed.analyzerWarningCount) analyzer warning(s)") }
    ...
    static func parseFallbackBuildIssue(_ line: String) -> BuildIssueObservation? {
        ...
        if prefixParts.count >= 3 {
            ...
            let location = path.isEmpty ? nil : SourceLocation(filePath: path, line: lineNumber, column: columnNumber)
