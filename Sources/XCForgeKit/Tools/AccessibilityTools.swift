...
public enum AccessibilityTools {
    ...
    static func accessibilityCheck(_ args: [String: Value]?, env: Environment) async -> CallTool.Result {
    ...
    private static func accessibilityCheckImpl(_ input: AccessibilityInput, env: Environment) async -> CallTool.Result {
        let sim: String
        do {
        ...
        for ss in screenshots {
            if ss.category == base.category {
                results.append(SizeCheckResult(
    ...
    private static func localizationCheckImpl(_ input: LocalizationInput, env: Environment) async -> CallTool.Result {
        let sim: String
        do {
            sim = try await env.session.resolveSimulator(input.simulator)
        } catch {
        ...
        if let bid = input.bundle_id, !bid.isEmpty {
            ...
            return .fail("No bundle_id provided and none cached from a previous build. Provide bundle_id.")
        }
        ...
        for ss in screenshots {
            ...
            if ss.locale == base.locale {
                results.append(LocaleCheckResult(
    ...
    private static func getCurrentContentSize(sim: String, env: Environment) async -> String? {
        ...
        guard let result = try? await env.shell.xcrun(
    ...
    private static func buildLocalizationReport(
        ...
        for r in results where !r.passed && !r.isBase {
               ...
               let data = try? FramebufferCapture.encodeImage(img, format: "jpeg", quality: 0.7)
            {
                ...
                content.append(.text(
                    text: "\(r.locale)\(r.isRTL ? " [RTL]" : "") — diff \(String(format: "%.2f%%", r.diffPercent))",
                    annotations: nil, _meta: nil
                ))
            }
        }
