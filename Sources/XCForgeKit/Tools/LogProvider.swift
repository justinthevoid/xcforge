...
public actor LogCapture {
  ...
  public private(set) var captureMode: String = "smart"

  public func start(arguments: [String], mode: String = "smart") throws {
...
public enum LogTools {
  ...
  static func parseLine(_ line: String) -> ParsedLogLine? {
    ...
    return ParsedLogLine(
      processName: processName, logType: logType,
  ...
  static let allTopics = [
    "app", "crashes", "network", "lifecycle", "springboard", "widgets", "background", "system",
  ]
  ...
  public struct TopicFilterResult {
    public let filteredLines: [String]
    public let topicCounts: [String: Int]
    public let totalLines: Int
  }

  /// Single-pass filter: categorize each line, count topics, collect matching lines.
  public static func filterByTopics(
    lines: [String],
    include: Set<String>,
    bundleId: String?,
    processName: String?
  ...
  public static func buildTopicSummary(
    ...
    lines.append(
      "--- \(result.totalLines) buffered, \(result.filteredLines.count) shown [\(shownTopics)] ---")
    ...
    let hiddenWithLines = allTopics.filter {
      !include.contains($0) && (result.topicCounts[$0] ?? 0) > 0
    }
    if !hiddenWithLines.isEmpty {
      ...
      lines.append(
        "Hint: include=[\(hiddenWithLines.prefix(3).map { "\"\($0)\"" }.joined(separator: ","))] to add \(hintTopics.joined(separator: ", ")) logs"
      )
    }
    ...
    if captureMode == "app" && include.count > 2 {
      lines.append(
        "WARNING: capture mode is 'app' — only app logs are in the stream. Use start_log_capture mode='smart' for topic filtering."
      )
    }
  ...
  public static func deriveProcessName(from appPath: String) -> String? {
    let plistPath =
      appPath.hasSuffix("/")
  ...
  public static func buildLogArgs(
    simulator: String,
    mode: String,
    level: String,
    process: String?,
    ...
    var args = [
      "simctl", "spawn", simulator, "log", "stream", "--style", "compact", "--level", level,
    ]
  ...
  public static let tools: [Tool] = [
      ...
      inputSchema: .object([
        ...
        "properties": .object([
          ...
          "mode": .object([
            "type": .string("string"),
            "description": .string(
              "Filter mode. 'app': only app logs + crashes (needs prior build_sim). 'smart': all logs minus noise. 'verbose': unfiltered."
            ),
          ...
          "process": .object([
            "type": .string("string"),
            "description": .string(
              "Filter by process name (uses efficient --process flag, server-side in logd). Bypasses mode logic."
            ),
          ]),
          "subsystem": .object([
            "type": .string("string"),
            "description": .string("Filter by subsystem, e.g. 'com.myapp'. Bypasses mode logic."),
          ]),
          "predicate": .object([
            "type": .string("string"),
            "description": .string("Custom NSPredicate filter. Bypasses mode logic."),
          ]),
          "level": .object([
            "type": .string("string"),
            "description": .string("Log level: default, info, debug. Default: debug"),
          ]),
        ]),
      ])
    ),
    Tool(
      ...
      description:
        "Stop the running log capture and release system resources. Any unread logs in the buffer are discarded. Call read_logs first if you need the captured data.",
      ...
      inputSchema: .object([
        ...
        "properties": .object([
          "include": .object([
            "type": .string("array"),
            "description": .string(
              "Topics to add: network, lifecycle, springboard, widgets, background, system. Default: app + crashes only."
            ),
          ...
          "clear": .object([
            "type": .string("boolean"),
            "description": .string("Clear buffer after reading. Default: false"),
          ]),
        ]),
      ])
    ),
    Tool(
      ...
      inputSchema: .object([
        ...
        "properties": .object([
          "pattern": .object([
            "type": .string("string"),
            "description": .string("Regex pattern to match in log lines"),
          ]),
          "timeout": .object([
            "type": .string("number"), "description": .string("Max seconds to wait. Default: 30"),
          ]),
          "simulator": .object([
            "type": .string("string"),
            "description": .string(
              "Simulator name or UDID. Auto-detected if omitted (used if log capture not running)."),
          ]),
          "subsystem": .object([
            "type": .string("string"),
            "description": .string("Filter by subsystem (used if log capture not running)"),
          ]),
        ]),
  ...
  struct StartLogInput: Decodable {
  ...
  struct ReadLogsInput: Decodable {
    let last: Int?
    let clear: Bool?
    // Custom decoding to handle array-or-string for `include`
    let include: IncludeTopics?

    struct IncludeTopics: Decodable {
      let values: [String]
  ...
  struct WaitForLogInput: Decodable {
    let pattern: String
    let timeout: Double?
    let simulator: String?
    let subsystem: String?
  ...
  static func startLogCapture(_ args: [String: Value]?, env: Environment) async -> CallTool.Result {
    switch ToolInput.decode(StartLogInput.self, from: args) {
  ...
  static func readLogs(_ args: [String: Value]?, env: Environment) async -> CallTool.Result {
    switch ToolInput.decode(ReadLogsInput.self, from: args) {
    case .failure(let err): return err
    case .success(let input): return await readLogsImpl(input, env: env)
    }
  }

  private static func readLogsImpl(_ input: ReadLogsInput, env: Environment) async
    -> CallTool.Result
  {
    let last = input.last
    ...
    if let topics = input.include {
      for s in topics.values { includeTopics.insert(s) }
    }
    ...
    if allLines.isEmpty {
      return .ok(
        "No log lines captured" + (isRunning ? " (capture is running)" : " (capture not running)"))
    }
  ...
  static func waitForLog(_ args: [String: Value]?, env: Environment) async -> CallTool.Result {
    switch ToolInput.decode(WaitForLogInput.self, from: args) {
    case .failure(let err): return err
    case .success(let input): return await waitForLogImpl(input, env: env)
    }
  }

  private static func waitForLogImpl(_ input: WaitForLogInput, env: Environment) async
    -> CallTool.Result
  {
    ...
    while CFAbsoluteTimeGetCurrent() < deadline {
      ...
      if !matchedLines.isEmpty {
        ...
        return .ok(
          "Pattern matched after \(elapsed)s (\(matchedLines.count) line(s)):\n\(truncated)")
      }
...
extension LogTools: ToolProvider {
  public static func dispatch(_ name: String, _ args: [String: Value]?, env: Environment) async
    -> CallTool.Result?
  {
    switch name {
    case "start_log_capture": return await startLogCapture(args, env: env)
    case "stop_log_capture": return await stopLogCapture(args)
    case "read_logs": return await readLogs(args, env: env)
    case "wait_for_log": return await waitForLog(args, env: env)
    default: return nil
    }
  }
}
