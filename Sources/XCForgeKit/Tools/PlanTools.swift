...
public enum PlanTools {
            ...
            inputSchema: .object([
                ...
                "properties": .object([
                    ...
                    "error_strategy": .object([
                        "type": .string("string"),
                        "description": .string("What to do on step failure: 'abort_with_screenshot' (default), 'abort', 'continue'"),
                    ]),
                    "timeout": .object([
                        "type": .string("number"),
        ...
        Tool(
            ...
            inputSchema: .object([
                ...
                "properties": .object([
                    ...
                    "decision": .object([
                        "type": .string("string"),
                        "description": .string("Your decision: 'accept', 'dismiss', 'skip', 'abort', or freeform guidance"),
                    ]),
                ]),
    ...
    public static func runPlan(_ args: [String: Value]?, env: Environment = .live) async -> CallTool.Result {
    ...
    public static func runPlanDecide(_ args: [String: Value]?, env: Environment = .live) async -> CallTool.Result {
    ...
    private static func runPlanDecideImpl(_ input: RunPlanDecideInput, env: Environment) async -> CallTool.Result {
        let sessionId = input.session_id
        let decision = input.decision

        // Consume session
        guard let suspended = await PlanSessionStore.shared.consume(sessionId) else {
            return .fail("Session '\(sessionId)' not found or expired (5-minute TTL). Re-run the plan to start fresh.")
        }
    ...
    private static func encodeReport(_ report: PlanReport) -> CallTool.Result {
        do {
            ...
            return .init(content: [.text(text: json, annotations: nil, _meta: nil)], isError: report.failed > 0)
        } catch {
            return .fail("Failed to encode report: \(error)")
        }
    }

    private static func encodeSuspendedReport(_ report: PlanReport, screenshot: String?) -> CallTool.Result {
        do {
            let json = try WorkflowJSONRenderer.renderJSON(report)
