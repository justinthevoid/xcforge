...
enum MultiDeviceTools {
  ...
  private static func log(_ msg: String) {
    let ts = String(
      format: "%.1f", CFAbsoluteTimeGetCurrent().truncatingRemainder(dividingBy: 10000))
    let line = "[\(ts)] \(msg)\n"
    let path = "/tmp/xcf-mdc-debug.log"
  ...
  struct MultiDeviceInput: Decodable {
    let app_path: String
  ...
  private static func multiDeviceCheckImpl(_ input: MultiDeviceInput, env: Environment) async
    -> CallTool.Result
  {
    let appPath = input.app_path
    guard !appPath.isEmpty else { return .fail("Missing required: app_path") }
    let simsStr = input.simulators
    guard !simsStr.isEmpty else { return .fail("Missing required: simulators") }
    ...
    let threshold = input.threshold ?? 1.0
    let testDarkMode = input.dark_mode ?? false
    let testLandscape = input.landscape ?? false
    ...
    log(
      "=== START multi_device_check run=\(runId) devices=\(simNames.count) dark=\(testDarkMode) landscape=\(testLandscape) ==="
    )
    ...
    do {
      let r = try await env.shell.xcrun(timeout: 15, "simctl", "list", "devices", "-j")
      guard r.succeeded,
    ...
    if !needsBoot.isEmpty {
      let bootResults = await withTaskGroup(of: TaskResult.self) { group in
        for device in needsBoot {
          ...
          group.addTask {
            do {
              let r = try await env.shell.xcrun(timeout: 60, "simctl", "boot", udid)
    ...
    let installResults = await withTaskGroup(of: TaskResult.self) { group in
      for device in bootedDevices {
        ...
        group.addTask {
          do {
            let r = try await env.shell.xcrun(timeout: 60, "simctl", "install", udid, appPath)
            return TaskResult(
              udid: udid, success: r.succeeded,
    ...
    if testLandscape {
      windowIDs = await resolveWindowIDs(deviceNames: readyDevices.map(\.name), env: env)
    ...
    for mode in modes {
      ...
      for d in readyDevices {
        _ = try? await env.shell.xcrun(
          timeout: 5, "simctl", "ui", d.udid, "appearance", mode.appearance)
      }
      ...
      if mode.orientation == "landscape" {
        for d in readyDevices {
          if let wid = windowIDs[d.name] {
            await rotateDevice(windowID: wid, name: d.name, toPortrait: false, env: env)
          } else {
      ...
      for d in readyDevices {
        ...
        let path = "/tmp/xcf-mdc-\(runId)-\(mode.label)-\(safeName).png"
        var ss = DeviceScreenshot(name: d.name, udid: d.udid, mode: mode)

        do {
          let r = try await env.shell.xcrun(
            timeout: 15, "simctl", "io", d.udid,
        ...
        for j in (i + 1)..<captured.count {
          ...
          if sameRes {
            ...
            if let diffImg = cmp.diffImage {
              let p =
                "/tmp/xcf-mdc-diff-\(runId)-\(mode.label)-\(VisualTools.sanitize(a.name))-vs-\(VisualTools.sanitize(b.name)).png"
    ...
    if testLandscape {
      for d in readyDevices {
        if let wid = windowIDs[d.name] {
          await rotateDevice(windowID: wid, name: d.name, toPortrait: true, env: env)
        }
      }
    }
      ...
      if !modePairs.isEmpty {
        for p in modePairs {
          if p.sameResolution {
            ...
            report.append(
              "  \(icon) \(p.nameA) vs \(p.nameB): \(String(format: "%.2f", p.diffPercent))%")
    ...
    let primaryShots = allScreenshots.filter {
      $0.mode.label == primaryMode.label && $0.image != nil
    }
    for ss in primaryShots {
      guard let img = ss.image else { continue }
      if let data = try? FramebufferCapture.encodeImage(
        scaleDown(img), format: "jpeg", quality: 0.6)
      {
        content.append(
          .image(
            data: data.base64EncodedString(), mimeType: "image/jpeg", annotations: nil, _meta: nil))
        content.append(
          .text(
            text: "\(ss.name) [\(primaryMode.label)] \(img.width)x\(img.height)", annotations: nil,
            _meta: nil))
      }
    }
    ...
    if testDarkMode {
      let darkShots = allScreenshots.filter {
        $0.mode.appearance == "dark" && $0.mode.orientation == "portrait" && $0.image != nil
      }
      for ss in darkShots {
        guard let img = ss.image else { continue }
        if let data = try? FramebufferCapture.encodeImage(
          scaleDown(img), format: "jpeg", quality: 0.6)
        {
          ...
          content.append(
            .text(
              text: "\(ss.name) [portrait-dark] \(img.width)x\(img.height)", annotations: nil,
              _meta: nil))
        }
      }
    }
    ...
    for p in allPairs where !p.passed && p.sameResolution {
        ...
        let data = try? FramebufferCapture.encodeImage(scaleDown(img), format: "jpeg", quality: 0.8)
      {
        ...
        content.append(
          .text(
            text: "DIFF [\(p.mode.label)]: \(p.nameA) vs \(p.nameB)", annotations: nil, _meta: nil))
      }
    }
  ...
  private static func resolveWindowIDs(deviceNames: [String], env: Environment) async -> [String:
    Int]
  {
    let script = """
      set output to ""
    ...
    guard
      let result = try? await env.shell.run(
        "/usr/bin/osascript", arguments: ["-e", script], timeout: 10),
      result.succeeded
    else { return [:] }
    ...
    for line in result.stdout.split(separator: "\n") {
      ...
      guard parts.count == 2, let wid = Int(parts[0].trimmingCharacters(in: .whitespaces)) else {
        continue
      }
  ...
  private static func rotateDevice(windowID: Int, name: String, toPortrait: Bool, env: Environment)
    async
  {
  ...
  private static func currentlyBootedUDIDs(env: Environment) async -> Set<String> {
    guard let result = try? await env.shell.xcrun(timeout: 10, "simctl", "list", "devices", "-j"),
  ...
  private static func waitForAllBooted(udids: [String], timeout: TimeInterval, env: Environment)
    async
  {
    let deadline = CFAbsoluteTimeGetCurrent() + timeout
    let target = Set(udids)
    while CFAbsoluteTimeGetCurrent() < deadline {
      let booted = await currentlyBootedUDIDs(env: env)
...
extension MultiDeviceTools: ToolProvider {
  public static func dispatch(_ name: String, _ args: [String: Value]?, env: Environment) async
    -> CallTool.Result?
  {
    switch name {
    case "multi_device_check": return await multiDeviceCheck(args, env: env)
    default: return nil
    }
  }
}
