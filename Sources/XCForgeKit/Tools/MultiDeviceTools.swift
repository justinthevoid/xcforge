...
enum MultiDeviceTools {
    ...
    private static func multiDeviceCheckImpl(_ input: MultiDeviceInput, env: Environment) async -> CallTool.Result {
        ...
        for i in devices.indices where devices[i].bootOk {
            ...
            _ = try? await env.shell.run("/usr/bin/xcrun",
                arguments: ["simctl", "terminate", udid, bundleId], timeout: 5)

            var launched = false
            for attempt in 1...2 {
                do {
                    let r = try await env.shell.run("/usr/bin/xcrun",
        ...
        for mode in modes {
            ...
            for d in readyDevices {
                _ = try? await env.shell.xcrun(timeout: 5, "simctl", "ui", d.udid, "appearance", mode.appearance)
            }
            ...
            for d in readyDevices {
                ...
                do {
                    let r = try await env.shell.xcrun(timeout: 15, "simctl", "io", d.udid,
                ...
                for j in (i + 1)..<captured.count {
                    ...
                    if sameRes {
                        ...
                        if let diffImg = cmp.diffImage {
                            let p = "/tmp/xcf-mdc-diff-\(runId)-\(mode.label)-\(VisualTools.sanitize(a.name))-vs-\(VisualTools.sanitize(b.name)).png"
    ...
    private static func resolveWindowIDs(deviceNames: [String], env: Environment) async -> [String: Int] {
        let script = """
        set output to ""
        ...
        return output
        """
        guard let result = try? await env.shell.run("/usr/bin/osascript", arguments: ["-e", script], timeout: 10),
    ...
    private static func rotateDevice(windowID: Int, name: String, toPortrait: Bool, env: Environment) async {
        ...
        let result = try? await env.shell.run("/usr/bin/osascript", arguments: ["-e", script], timeout: 10)
    ...
    private static func waitForAllBooted(udids: [String], timeout: TimeInterval, env: Environment) async {
