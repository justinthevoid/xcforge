...
- (NSDictionary *)fb_tree:(nullable NSSet<NSString *> *)excludedAttributes
{
  ...
  // why: XCUIApplication.snapshotWithError: only returns the keyWindow's hierarchy.
  // SwiftUI .sheet (and similar) mounts in a sibling UIWindow within the same scene,
  // so a single-snapshot tree silently omits sheet content. When the app exposes
  // more than one window, build a synthetic Application node whose children include
  // each window's snapshot. Single-window apps fall through unchanged.
  NSArray<id<FBXCElementSnapshot>> *windowSnapshots = [self.class fb_appWindowSnapshots:self];
  if (windowSnapshots.count > 1) {
    NSDictionary *appNode = [self.class dictionaryForElement:snapshot
                                                   recursive:NO
                                          excludedAttributes:excludedAttributes];
    NSMutableDictionary *merged = [appNode mutableCopy];
    NSMutableArray *children = [NSMutableArray array];
    for (id<FBXCElementSnapshot> winSnapshot in windowSnapshots) {
      @autoreleasepool {
        [children addObject:[self.class dictionaryForElement:winSnapshot
                                                   recursive:YES
                                          excludedAttributes:excludedAttributes]];
      }
    }
...
// Per-window snapshots used to expand `fb_tree:` and `fb_xmlRepresentationWithOptions:` when
// `windows.count > 1`. Returns nil/empty array when single-window, so callers can short-circuit
// to the existing single-snapshot path. Per-window snapshot failures are logged and skipped so
// that one bad window does not blank the whole tree.
+ (NSArray<id<FBXCElementSnapshot>> *)fb_appWindowSnapshots:(XCUIApplication *)app
{
  NSArray<XCUIElement *> *windows = nil;
  @try {
    windows = [app.windows allElementsBoundByAccessibilityElement];
  } @catch (NSException *e) {
    [FBLogger logFmt:@"fb_appWindowSnapshots: failed to enumerate windows: %@", e.reason];
    return @[];
  }
  if (windows.count <= 1) {
    return @[];
  }
  NSMutableArray<id<FBXCElementSnapshot>> *result = [NSMutableArray array];
  for (XCUIElement *window in windows) {
    @autoreleasepool {
      @try {
        id<FBXCElementSnapshot> snapshot = [window fb_standardSnapshot];
        if (nil != snapshot) {
          [result addObject:snapshot];
        }
      } @catch (NSException *e) {
        [FBLogger logFmt:@"fb_appWindowSnapshots: skipping a window snapshot due to exception: %@", e.reason];
      }
    }
  }
  return result.copy;
...
{
  // why: the single-snapshot path silently drops sibling UIWindows (e.g. SwiftUI .sheet).
  // Mirror fb_tree:'s gate — when more than one window exists, generate XML per-window
  // and stitch them under one synthetic Application root. Single-window apps are unchanged.
  NSArray<id<FBXCElementSnapshot>> *windowSnapshots = [self.class fb_appWindowSnapshots:self];
  if (windowSnapshots.count > 1) {
    NSString *merged = [self.class fb_mergedWindowsXMLRepresentation:windowSnapshots
                                                                 app:self
                                                             options:options];
    if (nil != merged) {
      return merged;
    }
    // Fall through to single-snapshot path on stitch failure.
  }
  return [FBXPath xmlStringWithRootElement:self options:options];
}

// Escape a string for safe inclusion as an XML attribute value. Handles the five
// XML predefined entities. Used by the multi-window stitcher to build the
// synthetic <XCUIElementTypeApplication ...> root attribute set.
static NSString *fb_xmlEscapedAttributeValue(NSString *value)
{
  if (nil == value) {
    return @"";
  }
  NSMutableString *escaped = [NSMutableString stringWithCapacity:value.length];
...
// Strip a leading `<?xml ... ?>` declaration (and any following whitespace/newlines)
// from a single XML document fragment so multiple fragments can embed under a wrapping
// root. Only inspects the first 200 characters to avoid pathological matches against
// `?>` sequences embedded deeper in attribute or text content.
static NSString *fb_xmlFragmentStripDeclaration(NSString *xml)
{
  if (nil == xml || ![xml hasPrefix:@"<?xml"]) {
    return xml;
  }
  NSUInteger searchLen = MIN((NSUInteger)200, xml.length);
  NSRange searchRange = NSMakeRange(0, searchLen);
  NSRange declEnd = [xml rangeOfString:@"?>" options:0 range:searchRange];
  if (declEnd.location == NSNotFound) {
    return xml;
  }
  NSString *remainder = [xml substringFromIndex:NSMaxRange(declEnd)];
  return [remainder stringByTrimmingCharactersInSet:[NSCharacterSet whitespaceAndNewlineCharacterSet]];
}

// Synthesize a multi-window XML document: emit each window snapshot via the existing
// FBXPath writer, then wrap the per-window fragments inside one synthetic Application
// element whose attributes mirror the real XCUIApplication snapshot (type/name/label/
// enabled/rect, etc.). Returns nil if any per-window write fails so the caller can
// fall back to the standard single-snapshot path. The `scope` option is applied only
// to the outer wrap; per-window fragments are emitted with `scope` cleared so they
// do not each get their own scope wrapper. All other options (notably
// `excludedAttributes`) are threaded through to per-window writes so multi-window
// output honors caller settings the same as single-window.
+ (nullable NSString *)fb_mergedWindowsXMLRepresentation:(NSArray<id<FBXCElementSnapshot>> *)windowSnapshots
                                                     app:(XCUIApplication *)app
                                                 options:(nullable FBXMLGenerationOptions *)options
{
  // Per-window options: copy of caller options with `scope` cleared so the scope wrap
  // is applied exactly once at the outer level. `excludedAttributes` etc. carry through.
  FBXMLGenerationOptions *perWindowOptions = nil;
  if (nil != options) {
    perWindowOptions = [[FBXMLGenerationOptions alloc] init];
    perWindowOptions.excludedAttributes = options.excludedAttributes;
    // scope intentionally left nil
  }
  NSMutableArray<NSString *> *windowFragments = [NSMutableArray array];
  for (id<FBXCElementSnapshot> winSnapshot in windowSnapshots) {
    @autoreleasepool {
      FBXCElementSnapshotWrapper *wrapped = [FBXCElementSnapshotWrapper ensureWrapped:winSnapshot];
      NSString *xml = [FBXPath xmlStringWithRootElement:(id)wrapped options:perWindowOptions];
      if (nil == xml) {
        return nil;
      }
      [windowFragments addObject:fb_xmlFragmentStripDeclaration(xml)];
    }
  }

  // Build the synthetic Application root attribute set from a real app snapshot so the
  // outer node is not a stripped-down placeholder. Mirror the same wd* properties the
  // JSON `dictionaryForElement:` path emits (type/name/label/enabled/value/rect).
  NSMutableString *appAttrs = [NSMutableString string];
  NSString *bundleId = app.bundleID ?: @"";
  // type
  [appAttrs appendFormat:@" type=\"%@\"", fb_xmlEscapedAttributeValue(@"XCUIElementTypeApplication")];
  // name = bundle id (preserves prior behavior; many clients rely on it)
  [appAttrs appendFormat:@" name=\"%@\"", fb_xmlEscapedAttributeValue(bundleId)];
  @try {
    id<FBXCElementSnapshot> appSnapshot = [app fb_standardSnapshot];
    if (nil != appSnapshot) {
      FBXCElementSnapshotWrapper *appWrapped = [FBXCElementSnapshotWrapper ensureWrapped:appSnapshot];
      NSString *appLabel = appWrapped.wdLabel;
      NSString *appValue = appWrapped.wdValue;
      if (nil != appLabel) {
        [appAttrs appendFormat:@" label=\"%@\"", fb_xmlEscapedAttributeValue(appLabel)];
      }
      if (nil != appValue) {
        [appAttrs appendFormat:@" value=\"%@\"", fb_xmlEscapedAttributeValue(appValue)];
      }
      [appAttrs appendFormat:@" enabled=\"%@\"", appWrapped.isWDEnabled ? @"true" : @"false"];
      NSDictionary *rect = appWrapped.wdRect;
      if (nil != rect) {
        [appAttrs appendFormat:@" x=\"%@\" y=\"%@\" width=\"%@\" height=\"%@\"",
         fb_xmlEscapedAttributeValue([rect[@"x"] description] ?: @"0"),
         fb_xmlEscapedAttributeValue([rect[@"y"] description] ?: @"0"),
         fb_xmlEscapedAttributeValue([rect[@"width"] description] ?: @"0"),
         fb_xmlEscapedAttributeValue([rect[@"height"] description] ?: @"0")];
      }
    }
  } @catch (NSException *e) {
    [FBLogger logFmt:@"fb_mergedWindowsXMLRepresentation: failed to read app snapshot for synthetic root: %@", e.reason];
  }
  ...
  [result appendFormat:@"<XCUIElementTypeApplication%@>\n", appAttrs];
  for (NSString *fragment in windowFragments) {
    [result appendString:fragment];
    [result appendString:@"\n"];
  }
  [result appendString:@"</XCUIElementTypeApplication>\n"];
  if (hasScope) {
    [result appendFormat:@"</%@>\n", options.scope];
  }
  return result.copy;
...
{
  // why: like fb_tree:/fb_xmlRepresentation*, the single-rooted description path silently
  // omits sibling UIWindows (e.g. SwiftUI .sheet). When more than one window exists,
  // emit a per-window section under a header so callers see all window content. Single-
  // window output remains byte-identical to the prior implementation.
  NSArray<id<FBXCElementSnapshot>> *windowSnapshots = [self.class fb_appWindowSnapshots:self];
  if (windowSnapshots.count > 1) {
    NSArray<XCUIElement *> *windowElements = nil;
    @try {
      windowElements = [self.windows allElementsBoundByAccessibilityElement];
    } @catch (NSException *e) {
      [FBLogger logFmt:@"fb_descriptionRepresentation: failed to enumerate windows: %@", e.reason];
      windowElements = @[];
    }
    NSMutableString *result = [NSMutableString string];
    NSString *bundleId = self.bundleID ?: @"";
    [result appendFormat:@"Application <%@> [%@ windows]:\n", bundleId, @(windowElements.count)];
    NSUInteger windowIdx = 0;
    for (XCUIElement *window in windowElements) {
      @autoreleasepool {
        [result appendFormat:@"\n--- Window %@ ---\n", @(windowIdx)];
        NSMutableArray<NSString *> *childDescs = [NSMutableArray array];
        @try {
          for (XCUIElement *child in [window.fb_query childrenMatchingType:XCUIElementTypeAny].allElementsBoundByIndex) {
            [childDescs addObject:child.debugDescription];
          }
        } @catch (NSException *e) {
          [FBLogger logFmt:@"fb_descriptionRepresentation: failed to enumerate window %@ children: %@", @(windowIdx), e.reason];
        }
        if (childDescs.count == 0) {
          [result appendString:window.debugDescription];
        } else {
          [result appendString:[childDescs componentsJoinedByString:@"\n\n"]];
        }
        windowIdx++;
      }
    }
