...
{
  XCUIElement *base = element;
  // why: when the coordinate base is the application root and the app has more than
  // one window (SwiftUI .sheet, system-styled modal, etc.), XCTest's hit-test would
  // route the synthesized touch through the keyWindow and never reach the on-top
  // sheet. Reroot on the topmost window so the tap lands where the user sees pixels.
  // Topmost = `windows.allElementsBoundByAccessibilityElement.lastObject`: XCUITest
  // orders the windows array back-to-front, so the last entry is on-top. This is the
  // load-bearing assumption — if Apple ever reverses ordering, this is the line to
  // revisit. We additionally require `exists && isHittable` on the candidate window
  // so a hidden/offscreen sibling does not become the tap base; if the topmost fails
  // either check we walk back-to-front and pick the first window that does qualify.
  // If none qualify, fall through to the original element (no rerooting).
  // Element-targeted callers pass a real element (not XCUIApplication), so the guard
  // keeps that path identical to current behavior.
  if ([element isKindOfClass:XCUIApplication.class]) {
    NSArray<XCUIElement *> *windows = nil;
    @try {
      windows = [(XCUIApplication *)element windows].allElementsBoundByAccessibilityElement;
    } @catch (NSException *e) {
      [FBLogger logFmt:@"gestureCoordinateWithOffset: window enumeration failed: %@", e.reason];
      windows = nil;
    }
    if (windows.count > 1) {
      XCUIElement *chosen = nil;
      for (NSInteger i = (NSInteger)windows.count - 1; i >= 0; i--) {
        XCUIElement *candidate = windows[(NSUInteger)i];
        @try {
          ...
          [FBLogger logFmt:@"gestureCoordinateWithOffset: window hittable check failed: %@", e.reason];
        }
      }
      if (nil != chosen) {
        base = chosen;
  ...
  return [[base coordinateWithNormalizedOffset:CGVectorMake(0, 0)] coordinateWithOffset:offset];
}
