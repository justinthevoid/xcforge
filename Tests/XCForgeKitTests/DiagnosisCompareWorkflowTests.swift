...
struct DiagnosisCompareWorkflowTests {
  ...
  func unchangedTestRerunStaysCompactWhenFailureSignalIsTheSame() async throws {
    ...
    #expect(
      result.unchangedBlockers.contains(
        "Primary test remains: AppTests/LoginTests/testShowsErrorBanner() - XCTAssertEqual failed"))
  ...
  func partialBuildImprovementKeepsTheRemainingBlockersExplicit() async throws {
    ...
    let sourceXCResult = tempDir.appendingPathComponent(
      "source-partial.xcresult", isDirectory: true)
    ...
    #expect(
      result.unchangedBlockers.contains(
        "Primary build signal remains: Cannot find 'WidgetView' in scope"))
  }
  ...
  func failedToPartialRerunStaysPartialAndReportsEvidenceChanges() async throws {
    ...
    let store = RunStore(baseDirectory: tempDir)
    let sourceXCResult = tempDir.appendingPathComponent(
      "source-partial-status.xcresult", isDirectory: true)
    let rerunXCResult = tempDir.appendingPathComponent(
      "rerun-partial-status.xcresult", isDirectory: true)
    try FileManager.default.createDirectory(
      at: sourceXCResult, withIntermediateDirectories: true, attributes: nil)
    try FileManager.default.createDirectory(
      at: rerunXCResult, withIntermediateDirectories: true, attributes: nil)
    ...
    let run = makeTestComparisonRun(
      ...
      sourceSummary: makeTestSummary(
        ...
        expectedFailureCount: 0,
        inferredSummary:
          "The run is blocked by failing test AppTests/LoginTests/testShowsErrorBanner(): XCTAssertEqual failed",
        supportingEvidencePath: sourceXCResult.path
      ),
      currentSummary: makeTestSummary(
        ...
        expectedFailureCount: 0,
        inferredSummary:
          "The rerun still reports the same failing test after improving evidence capture.",
        supportingEvidencePath: rerunXCResult.path
      ),
      sourceEvidencePath: sourceXCResult.path,
      currentEvidencePath: rerunXCResult.path,
  ...
  func regressedComparisonFlagsTheFollowUpAttemptAsWorseThanThePriorSuccess() async throws {
    ...
    let store = RunStore(baseDirectory: tempDir)
    let sourceXCResult = tempDir.appendingPathComponent(
      "source-regressed.xcresult", isDirectory: true)
    let rerunXCResult = tempDir.appendingPathComponent(
      "rerun-regressed.xcresult", isDirectory: true)
    try FileManager.default.createDirectory(
      at: sourceXCResult, withIntermediateDirectories: true, attributes: nil)
    try FileManager.default.createDirectory(
      at: rerunXCResult, withIntermediateDirectories: true, attributes: nil)
  ...
  func multiRerunChainsCompareTheLatestAttemptAgainstTheOriginalSourceAttempt() async throws {
    ...
    try FileManager.default.createDirectory(
      at: sourceXCResult, withIntermediateDirectories: true, attributes: nil)
    try FileManager.default.createDirectory(
      at: middleXCResult, withIntermediateDirectories: true, attributes: nil)
    try FileManager.default.createDirectory(
      at: latestXCResult, withIntermediateDirectories: true, attributes: nil)
    ...
    let run = WorkflowRunRecord(
      ...
      evidence: [
        WorkflowEvidenceRecord(
          kind: .buildSummary, phase: .diagnosisBuild, attemptId: "attempt-1", attemptNumber: 1,
          availability: .available, unavailableReason: nil,
          reference: "run_record.diagnosisSummary", source: "xcforge.diagnosis_build.summary"),
        WorkflowEvidenceRecord(
          kind: .xcresult, phase: .diagnosisBuild, attemptId: "attempt-1", attemptNumber: 1,
          availability: .available, unavailableReason: nil, reference: sourceXCResult.path,
          source: "xcodebuild.result_bundle"),
        WorkflowEvidenceRecord(
          kind: .buildSummary, phase: .diagnosisBuild, attemptId: "attempt-2", attemptNumber: 2,
          availability: .available, unavailableReason: nil,
          reference: "run_record.diagnosisSummary", source: "xcforge.diagnosis_build.summary"),
        WorkflowEvidenceRecord(
          kind: .xcresult, phase: .diagnosisBuild, attemptId: "attempt-2", attemptNumber: 2,
          availability: .available, unavailableReason: nil, reference: middleXCResult.path,
          source: "xcodebuild.result_bundle"),
        WorkflowEvidenceRecord(
          kind: .buildSummary, phase: .diagnosisBuild, attemptId: "attempt-3", attemptNumber: 3,
          availability: .available, unavailableReason: nil,
          reference: "run_record.diagnosisSummary", source: "xcforge.diagnosis_build.summary"),
        WorkflowEvidenceRecord(
          kind: .xcresult, phase: .diagnosisBuild, attemptId: "attempt-3", attemptNumber: 3,
          availability: .available, unavailableReason: nil, reference: latestXCResult.path,
          source: "xcodebuild.result_bundle"),
      ],
      attemptHistory: [
        WorkflowAttemptSnapshot(
          attempt: attempt1, phase: .diagnosisBuild, status: .failed,
          resolvedContext: makeResolvedContext(), diagnosisSummary: sourceSummary,
          recordedAt: Date(timeIntervalSince1970: 1_743_700_000)),
        WorkflowAttemptSnapshot(
          attempt: attempt2, phase: .diagnosisBuild, status: .failed,
          resolvedContext: makeResolvedContext(), diagnosisSummary: middleSummary,
          recordedAt: Date(timeIntervalSince1970: 1_743_700_100)),
        WorkflowAttemptSnapshot(
          attempt: attempt3, phase: .diagnosisBuild, status: .succeeded,
          resolvedContext: makeResolvedContext(), diagnosisSummary: latestSummary,
          recordedAt: Date(timeIntervalSince1970: 1_743_700_200)),
      ]
    )
    _ = try store.save(run)
  ...
  private func makeBuildSummary(
    ...
    BuildDiagnosisSummary(
      ...
      supportingEvidence: [
        EvidenceReference(
          kind: "xcresult", path: supportingEvidencePath, source: "xcodebuild.result_bundle")
      ]
    )
  }
