...
struct DiagnosisRuntimeWorkflowTests {
  ...
  func successfulRuntimeCapturePersistsSummary() async throws {
    ...
    #expect(
      persisted.evidence.contains(where: {
        $0.kind == .runtimeSummary && $0.availability == .available
      }))
  ...
  func supportedRuntimeInterruptionsRetryAndRecordRecoveryHistory() async throws {
    ...
    let ids = TestIDGenerator([
      "attempt-runtime-partial", "attempt-runtime-recovery", "recovery-record",
    ])
    ...
    let hasInitialEvidence = persisted.evidence.contains(where: {
      $0.attemptId == "attempt-runtime-partial"
    })
  ...
  func staleSimulatorContinuityRetriesAndRecordsRecoveryHistory() async throws {
    ...
    _ = try store.save(run)
    let ids = TestIDGenerator([
      "attempt-runtime-stale", "attempt-runtime-stale-recovery", "recovery-record-stale",
    ])
    let captureCount = RuntimeCaptureCounter()
  ...
  func failedRecoveryRecordsFailedAttemptAndPreservesPartialEvidence() async throws {
    ...
    let recoverySnapshot = try #require(
      persisted.attemptSnapshot(forAttemptId: "attempt-runtime-recovery", phase: .diagnosisRuntime))
  ...
  func unsupportedRequestedScreenshotCaptureIsPartial() async throws {
    ...
    #expect(
      result.failure?.message
        == "Screen recording permission is unavailable for this simulator screenshot.")
