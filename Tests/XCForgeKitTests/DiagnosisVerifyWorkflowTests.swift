...
struct DiagnosisVerifyWorkflowTests {
    ...
    func rerunsFailedTestAndMarksVerified() async throws {
        ...
        try FileManager.default.createDirectory(at: rerunXCResultURL, withIntermediateDirectories: true, attributes: nil)
        ...
        #expect(result.evidence.contains(where: { $0.kind == .xcresult && $0.reference == rerunXCResultURL.path }))
    ...
    func rerunOverridesResolveNewContextAndBlockedRerunsStayExplicit() async throws {
        ...
        let rerunXCResultURL = tempDir.appendingPathComponent("blocked-rerun.xcresult", isDirectory: true)
        let rerunStderrURL = tempDir.appendingPathComponent("blocked-rerun.stderr.txt", isDirectory: false)
        try FileManager.default.createDirectory(at: rerunXCResultURL, withIntermediateDirectories: true, attributes: nil)
        ...
        let workflow = DiagnosisVerifyWorkflow(
            ...
            executeTest: { context in
                ...
                return TestTools.TestDiagnosisExecution(
                    ...
                    executionFailureMessage: "Unable to find a destination matching the provided destination specifier"
                )
            },
        ...
        WorkflowRunRecord(
            ...
            testDiagnosisSummary: TestDiagnosisSummary(
                observedEvidence: ObservedTestEvidence(
                    summary: "Primary failing test selected from 1 failing test(s): AppTests/LoginTests/testShowsErrorBanner().",
                ...
                inferredConclusion: InferredTestConclusion(
                    summary: "The run appears primarily blocked by failing test AppTests/LoginTests/testShowsErrorBanner(): XCTAssertEqual failed"
                ),
                supportingEvidence: [
                    EvidenceReference(kind: "xcresult", path: "/tmp/original-test.xcresult", source: "xcodebuild.result_bundle")
                ]
            ),
            evidence: [
    ...
    private func makeFailedBuildRun(runId: String = "run-build") -> WorkflowRunRecord {
        WorkflowRunRecord(
            ...
            diagnosisSummary: BuildDiagnosisSummary(
                ...
                supportingEvidence: [
                    EvidenceReference(kind: "xcresult", path: "/tmp/original-build.xcresult", source: "xcodebuild.result_bundle")
                ]
            ),
            evidence: [
                WorkflowEvidenceRecord(
