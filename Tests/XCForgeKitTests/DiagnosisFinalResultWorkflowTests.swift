...
struct DiagnosisFinalResultWorkflowTests {
  ...
  func rerunComparisonIncludesMeaningfulChangeAndBothEvidenceBundles() async throws {
    ...
    #expect(
      result.comparison?.changedEvidence.contains(where: { $0.field == "Overall status" }) == true)
    #expect(result.currentAttempt?.availableEvidence.count == 2)
  ...
  func rerunWithoutLineageDegradesToThePersistedTruth() async throws {
    ...
    let store = RunStore(baseDirectory: tempDir)
    let xcresultURL = tempDir.appendingPathComponent(
      "rerun-missing-source.xcresult", isDirectory: true)
    try FileManager.default.createDirectory(
      at: xcresultURL, withIntermediateDirectories: true, attributes: nil)
    ...
    #expect(
      result.comparisonNote
        == "Run run-final-missing-lineage does not include a linked rerun attempt to compare.")
  }
  ...
  func unchangedRerunsKeepTheirBlockersExplicitInTheFinalResult() async throws {
    ...
    let sourceXCResult = tempDir.appendingPathComponent(
      "source-unchanged.xcresult", isDirectory: true)
    let rerunXCResult = tempDir.appendingPathComponent(
      "rerun-unchanged.xcresult", isDirectory: true)
    try FileManager.default.createDirectory(
      at: sourceXCResult, withIntermediateDirectories: true, attributes: nil)
    try FileManager.default.createDirectory(
      at: rerunXCResult, withIntermediateDirectories: true, attributes: nil)
    ...
    #expect(
      result.comparison?.unchangedBlockers.contains(
        "Primary build signal remains: Cannot find 'WidgetView' in scope") == true)
    #expect(result.failure == nil)
  }
  ...
  func finalResultsPreserveRecoveryHistoryForRecoveredRuntimeRuns() async throws {
    ...
    let run = WorkflowRunRecord(
      ...
      runtimeSummary: RuntimeDiagnosisSummary(
        ...
        supportingEvidence: [
          EvidenceReference(
            kind: "console_log", path: runtimeLogURL.path, source: "simctl.launch_console")
        ]
      ),
    ...
    #expect(
      result.recoveryHistory.first?.summary
  ...
  func missingOrNonTerminalRunsFailExplicitly() async throws {
    ...
    let missingResult = await missingWorkflow.assemble(
      request: DiagnosisFinalResultRequest(runId: "missing-run"))
    ...
    let result = await workflow.assemble(
      request: DiagnosisFinalResultRequest(runId: nonTerminalRun.runId))
  ...
  func omittedRunIdPrefersNewestTerminalDiagnosisRun() async throws {
    ...
    let store = RunStore(baseDirectory: tempDir)
    let terminalXCResult = tempDir.appendingPathComponent(
      "terminal-build.xcresult", isDirectory: true)
    try FileManager.default.createDirectory(
      at: terminalXCResult, withIntermediateDirectories: true, attributes: nil)
