...
struct DiagnosisBuildWorkflowTests {
  ...
  func failingBuildPersistsSummary() async throws {
    ...
    #expect(
      result.summary?.observedEvidence.primarySignal?.message == "Cannot find 'WidgetView' in scope"
    )
    #expect(
      result.summary?.observedEvidence.primarySignal?.location
        == SourceLocation(
    ...
    #expect(
      result.summary?.inferredConclusion?.summary.contains("Cannot find 'WidgetView' in scope")
        == true)
