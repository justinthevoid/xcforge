...
struct DiagnosisFinalResultRendererTests {
  ...
  func renderKeepsRecoveryAndRerunReviewDetailsBelowTheSummary() {
    ...
    #expect(
      rendered.contains("Linked this terminal result to source attempt attempt-2 for rerun review.")
    )
  ...
  @Test(
    "render uses explicit textual fallback when proof is missing or the result failed before assembly"
  )
  func renderUsesExplicitTextualFallbackWhenProofIsMissingOrTheResultFailedBeforeAssembly() {
    ...
    #expect(
      missingProofRendered.contains(
        "  proof_cue: Best available artifact is missing: screenshot for attempt 4 (unsupported)."))
    #expect(
      missingProofRendered.contains(
        "  next_review: Review missing evidence notes in Current Evidence Bundle below."))

    #expect(failedAssemblyRendered.contains("  outcome: Result unavailable"))
    #expect(
      failedAssemblyRendered.contains(
        "  finding: No diagnosis run was found for run ID missing-run."))
  ...
  func renderSeparatesTestObservedEvidenceFromInferredConclusion() {
    ...
    #expect(
      rendered.contains(
        "    failure_message: Expected view to contain 'Hello' but found empty view."))
    #expect(rendered.contains("    counts: total=10, failed=3, passed=7, skipped=0"))
    #expect(rendered.contains("  Test Inferred Conclusion"))
    #expect(
      rendered.contains("    summary: Widget rendering tests regressed after the layout refactor."))
    assertSectionOrder(
      rendered,
      sections: [
        "Investigation Summary",
  ...
  func renderPresentsGuidanceBlockStructurallySeparateFromDiagnosisAndEvidence() {
    ...
    #expect(
      rendered.contains(
        "  suggested_action: Review evidence to confirm the result meets expectations."))
  ...
  func renderPresentsFollowOnGuidanceForFailureWithActionOrientedLabel() {
    ...
    #expect(
      rendered.contains(
        "  suggested_action: Review the failure details and address the root cause before retrying."
      ))
  ...
  func renderWithNarrowLayoutUsesStackedEvidenceFormatAndStatusPrefix() {
    let result = DiagnosisFinalResult(
      ...
      currentAttempt: makeAttempt(
        ...
        status: .failed,
        summary: DiagnosisStatusSummary(
          source: .build, headline: "Build failed.", detail: "Missing symbol"),
        evidence: [
          makeEvidence(
            kind: .xcresult,
            phase: .diagnosisBuild,
    ...
    let rendered = DiagnosisFinalResultRenderer.render(
      result.withDerivedFollowOnAction(), layout: .narrow)
  ...
  func renderWithMediumLayoutWrapsEvidenceLinesAndIncludesStatusPrefix() {
    ...
    let rendered = DiagnosisFinalResultRenderer.render(
      result.withDerivedFollowOnAction(), layout: .medium)
  ...
  func renderIncludesStatusPrefixForAllTerminalStates() {
    ...
    let unsupportedRendered = DiagnosisFinalResultRenderer.render(
      unsupportedResult, layout: .narrow)
...
struct WorkflowPresentationHelpersFollowOnGuidanceTests {
  ...
  func followOnActionForRerunWithComparison() {
    let result = DiagnosisFinalResult(
      ...
      comparison: DiagnosisFinalComparison(
        ...
        changedEvidence: [
          DiagnosisComparisonChange(
            field: "status", priorValue: "failed", currentValue: "succeeded")
        ],
        unchangedBlockers: []
      ),
      failure: nil,
    ...
    #expect(guidance != nil)
    #expect(
      guidance?.action == "Review the comparison to confirm the change had the expected effect.")
    #expect(guidance?.confidence == .evidenceSupported)
  }
  ...
  func followOnActionForFailureObjectDirectsToRootCause() {
    ...
    #expect(guidance != nil)
    #expect(
      guidance?.action == "Review the failure details and address the root cause before retrying.")
