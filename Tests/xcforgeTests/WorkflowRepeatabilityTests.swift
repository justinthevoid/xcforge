...
struct WorkflowRepeatabilityTests {
    ...
    func schemaVersionConsistency() throws {
        ...
        let evidenceJSON = try Self.parseJSON(WorkflowJSONRenderer.renderJSON(Self.makeEvidenceResult()))
    ...
    @Test("WorkflowStatus raw values are stable in JSON output", arguments: [
    ...
    @Test("WorkflowFailureClassification raw values are stable in JSON output", arguments: [
    ...
    @Test("WorkflowPhase raw values are stable in JSON output", arguments: [
    ...
    func contractTypeCountGuard() {
        #expect(StableWorkflowContract.allSupportedResultTypes.count == StableWorkflowContract.supportedResultTypeCount)
    }
}
