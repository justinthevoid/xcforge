...
struct DiagnosisTestWorkflowTests {
    ...
    func failingTestsPersistSummary() async throws {
        ...
        #expect(result.summary?.observedEvidence.primaryFailure?.testIdentifier == "AppTests/LoginTests/testShowsErrorBanner()")
        #expect(result.summary?.observedEvidence.primaryFailure?.message == "XCTAssertEqual failed: (\"Welcome\") is not equal to (\"Error\")")
    ...
    func successfulTestsReportNoFailureSignal() async throws {
        ...
        #expect(result.summary?.inferredConclusion?.summary == "No failing test signal was found for this run.")
    ...
    func environmentBlockersReturnUnsupported() async throws {
        ...
        try FileManager.default.createDirectory(at: xcresultURL, withIntermediateDirectories: true, attributes: nil)
        let stderrURL = tempDir.appendingPathComponent("test-blocked.xcresult.stderr.txt", isDirectory: false)
    ...
    func testExecutionErrorsPersistFailedRunState() async throws {
        ...
        let workflow = DiagnosisTestWorkflow(
            ...
            persistRun: { run in try store.update(run) },
            executeTest: { _ in
                throw SmartContextError("xcodebuild test invocation crashed")
            }
        )
        ...
        #expect(persisted.evidence == [
            WorkflowEvidenceRecord(
                ...
                reference: nil,
                source: "xcforge.diagnosis_test.summary",
                detail: "Test execution failed before xcforge could persist a test summary. xcodebuild test invocation crashed"
            ),
            WorkflowEvidenceRecord(
                kind: .xcresult,
                phase: .diagnosisTest,
