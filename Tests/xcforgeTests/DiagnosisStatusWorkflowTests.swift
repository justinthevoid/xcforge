...
struct DiagnosisStatusWorkflowTests {
    ...
    func explicitRunLookupReturnsPersistedBuildStatus() async throws {
        ...
        let run = makeRun(
            ...
            diagnosisSummary: BuildDiagnosisSummary(
                ...
                supportingEvidence: [
                    EvidenceReference(kind: "xcresult", path: "/tmp/build-run.xcresult", source: "xcodebuild.result_bundle")
                ]
            )
        )
    ...
    func statusInspectionKeepsPreparedSimulatorContextVisible() async throws {
        ...
        #expect(result.resolvedContext?.simulatorPreparation == makePreparedSimulatorContext(requested: "iPhone 16 Pro", selected: "SIM-123"))
    ...
    func evidenceInspectionReturnsGroupedBuildEvidence() async throws {
        ...
        #expect(result.buildSummary?.observedEvidence.summary == "Build failed with a primary compiler error.")
        #expect(result.buildSummary?.inferredConclusion?.summary == "The run failed because WidgetView is unresolved.")
    ...
    func evidenceInspectionKeepsBuildAndTestEvidenceSeparate() async throws {
        ...
        #expect(result.buildSummary?.observedEvidence.summary == "Build succeeded without an error diagnostic.")
        #expect(result.buildSummary?.inferredConclusion?.summary == "No build failure signal was found for this run.")
        #expect(result.testSummary?.observedEvidence.summary == "Test run completed without a failing test signal.")
        #expect(result.testSummary?.inferredConclusion?.summary == "No failing test signal was found for this run.")
    ...
    func startPhaseRunsReportWithoutFabricatedSummary() async throws {
        ...
        #expect(result.summary?.source == .start)
        #expect(result.summary?.headline == "Diagnosis run is in phase diagnosis_start with status in_progress.")
    ...
    func terminalDiagnosisTestRunsExposePersistedSummary() async throws {
        ...
        let run = makeRun(
            ...
            testDiagnosisSummary: TestDiagnosisSummary(
                ...
                supportingEvidence: [
                    EvidenceReference(kind: "stderr", path: "/tmp/test-blocked.stderr.txt", source: "xcodebuild.stderr")
                ]
            )
        )
        _ = try store.save(run)
        ...
        #expect(result.summary?.headline == "Unable to find a destination matching the provided destination specifier")
        #expect(result.summary?.detail == "Test execution was blocked before a failing test could be identified.")
    }
    ...
    func runtimePhaseRunsExposePersistedRuntimeSummary() async throws {
        ...
        let run = makeRun(
            ...
            runtimeSummary: makeRuntimeSummary(
                observedSummary: "App com.example.app launched, but runtime inspection ended before the app stayed running.",
                inferredSummary: "The app reached launch, but the runtime state remained unstable or exited before the capture window completed."
            ),
            evidence: [
                WorkflowEvidenceRecord(
                    kind: .runtimeSummary,
        ...
        let evidenceResult = await workflow.inspectEvidence(request: DiagnosisStatusRequest(runId: run.runId))

        #expect(statusResult.isSuccessfulInspection)
        #expect(statusResult.summary?.source == .runtime)
        #expect(statusResult.summary?.headline == "App com.example.app launched, but runtime inspection ended before the app stayed running.")
        #expect(statusResult.summary?.detail == "launchctl print stalled")
        #expect(evidenceResult.isSuccessfulInspection)
        #expect(evidenceResult.runtimeSummary?.observedEvidence.summary == "App com.example.app launched, but runtime inspection ended before the app stayed running.")
        #expect(evidenceResult.runtimeSummary?.inferredConclusion?.summary == "The app reached launch, but the runtime state remained unstable or exited before the capture window completed.")
        #expect(evidenceResult.evidenceState == .partial)
        let screenshotEvidence = evidenceResult.availableEvidence.first(where: { $0.kind == .screenshot })
    ...
    func runtimeEvidenceInspectionKeepsUnavailableScreenshotGapsVisible() async throws {
        ...
        let run = makeRun(
            ...
            runtimeSummary: makeRuntimeSummary(
                ...
                inferredSummary: "Runtime inspection reached a running app state with captured console output or a confirmed live console session."
            ),
            evidence: [
                WorkflowEvidenceRecord(
                    kind: .runtimeSummary,
        ...
        #expect(evidenceResult.isSuccessfulInspection)
        #expect(evidenceResult.evidenceState == .partial)
        let unavailableScreenshot = evidenceResult.unavailableEvidence.first(where: { $0.kind == .screenshot })
        #expect(unavailableScreenshot?.producingWorkflowStep == "runtime diagnosis")
        #expect(unavailableScreenshot?.detail == "simctl io screenshot exited before writing the artifact.")
    }
    ...
    func statusAndEvidenceInspectionKeepRecoveryNarrativeVisible() async throws {
        ...
        #expect(statusResult.recoveryHistory.first?.summary == "Recovered from broken launch continuity and resumed runtime diagnosis.")
        #expect(evidenceResult.isSuccessfulInspection)
    ...
    private func makeBuildSummary(observedSummary: String, inferredSummary: String) -> BuildDiagnosisSummary {
    ...
    private func makeTestSummary(observedSummary: String, inferredSummary: String) -> TestDiagnosisSummary {
        TestDiagnosisSummary(
            ...
            supportingEvidence: [
                EvidenceReference(kind: "xcresult", path: "/tmp/test-run.xcresult", source: "xcodebuild.result_bundle")
            ]
        )
    }

    private func makeRuntimeSummary(observedSummary: String, inferredSummary: String) -> RuntimeDiagnosisSummary {
        RuntimeDiagnosisSummary(
            ...
            supportingEvidence: [
                EvidenceReference(kind: "console_log", path: "/tmp/runtime-console.log", source: "simctl.launch_console")
            ]
        )
    }
