...
struct DiagnosisStartWorkflowTests {
    ...
    func successfulStartPersistsRun() async throws {
        ...
        #expect(result.resolvedContext == ResolvedWorkflowContext(
        ...
        #expect(result.resolvedContext?.simulatorPreparation == makePreparedSimulatorContext(requested: "SIM-123"))
        ...
        #expect(persisted.resolvedContext.simulatorPreparation == makePreparedSimulatorContext(requested: "SIM-123"))
    ...
    func omittedFieldsReuseLatestActiveContext() async throws {
        ...
        #expect(result.contextProvenance?.fields.map(\.source) == [
            .reusedRun,
            .reusedRun,
            .reusedRun,
            .reusedRun,
    ...
    func explicitOverridePreservesReuseProvenance() async throws {
        ...
        #expect(result.contextProvenance?.fields.first(where: { $0.field == .project })?.source == .reusedRun)
        #expect(result.contextProvenance?.fields.first(where: { $0.field == .scheme })?.source == .explicit)
        #expect(result.contextProvenance?.fields.first(where: { $0.field == .simulator })?.source == .reusedRun)
        #expect(result.contextProvenance?.fields.first(where: { $0.field == .build })?.source == .reusedRun)
