...
const remediationMarkdownPath = join(
	...
	'homepage-redesign-remediation-tasks.md',
);
const remediationJsonPath = join(
	repoRoot,
	'_bmad-output',
	'implementation-artifacts',
	'homepage-redesign-remediation-tasks.json',
);
...
function validateAssistiveTechContracts({ heroSource, finalSource }) {
	const storyScope = 'assistive-tech-contracts';

	// Hero: install terminal block must have an accessible label.
	expectIncludes(
		heroSource,
		'aria-label="Install commands"',
		storyScope,
		paths.hero,
		'Hero install terminal block is missing aria-label for assistive technologies.',
		'Keep aria-label="Install commands" on the install-terminal div.',
	);

	// Hero: prompt characters must be hidden from screen readers.
	expectIncludes(
		heroSource,
		'aria-hidden="true"',
		storyScope,
		paths.hero,
		'Hero terminal prompt characters are missing aria-hidden.',
		'Keep aria-hidden="true" on install-terminal-prompt spans.',
	);

	// FinalCTA: install command pre must be labeled and focusable.
	expectMatches(
		finalSource,
	...
	// FinalCTA: status message must have role="status" and aria-atomic="true".
	expectMatches(
		finalSource,
	...
	// FinalCTA: install handoff must announce updates with aria-live.
	expectIncludes(
		finalSource,
		'aria-live="polite"',
		storyScope,
		paths.finalCta,
		'Install handoff region is missing aria-live="polite".',
		'Keep aria-live="polite" on the install-handoff section for polite state announcements.',
	);
}
	...
	for (const pair of requiredPairs) {
		...
		if (foregroundColor.alpha < 1 || backgroundColor.alpha < 1) {
			fail(
				...
				'Use opaque hex color tokens for required contrast pairs.',
			);
			continue;
		}
...
function validateMotionSafeContracts(primitivesSource, homepageSource) {
	...
	if (homepageReducedMotionBlocks.length === 0) {
		fail(
			...
			'Add reduced-motion overrides for hero and final CTA modules.',
		);
		return;
	}
	...
	// These selectors must appear in the reduced-motion block with transition: none.
	...
	// These selectors must appear with transform: none.
	...
	for (const selector of homepageTransitionSelectors) {
		if (!homepageReducedMotionSource.includes(selector)) {
			fail(
				...
				paths.homepage,
				`Reduced-motion block is missing selector ${selector}.`,
				'Restore required selector coverage to keep CTA motion-safe behavior.',
			);
		}
	...
	// Feature card hover transform must also be neutralized.
	if (!homepageReducedMotionSource.includes('.feature-card:hover')) {
		fail(
			scope,
			paths.homepage,
			'Reduced-motion block is missing .feature-card:hover transform neutralization.',
			'Add .feature-card:hover { transform: none; } in the reduced-motion block.',
		);
	}
}
...
function writeRemediationArtifact() {
	...
	const markdownLines = [
		'# Homepage Redesign Remediation Tasks',
		'',
...
function reportAndExitIfFailures() {
	if (failures.length === 0) {
		...
		console.log('[assistive-tech-contrast-motion] Assistive-tech, contrast, and motion-safe checks passed.');
		return;
	}

	writeRemediationArtifact();

	console.error('[assistive-tech-contrast-motion] Validation failed.');
	console.error('[assistive-tech-contrast-motion] Remediation guidance follows:');
	failures.forEach((failure, index) => {
		console.error(
	...
	console.error(
		`[assistive-tech-contrast-motion] Remediation tasks written to ${relFromRepo(remediationMarkdownPath)} and ${relFromRepo(remediationJsonPath)}.`,
	);
	process.exit(1);
}
...
function main() {
	...
	if (heroSource && finalSource) {
		validateAssistiveTechContracts({ heroSource, finalSource });
	}
