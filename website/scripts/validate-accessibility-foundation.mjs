...
function readRequiredFile(filePath, scope) {
  if (!existsSync(filePath)) {
    fail(scope, filePath, 'Required file is missing.', 'Restore the file and re-run the accessibility foundation validator.');
    return null;
  }
  ...
  try {
    ...
    if (!source.trim()) {
      fail(scope, filePath, 'Required file is empty.', 'Restore the expected implementation content before building.');
      return null;
    }
    ...
    fail(scope, filePath, `Unable to read file. ${message}`, 'Resolve filesystem/read issues and retry validation.');
    return null;
  }
}
...
try {
  ...
  const toggleSource = readRequiredFile(componentPaths.proofToggle, `component:ProofToggle (${routeId})`);
  const docsSource = readRequiredFile(componentPaths.docsHandoff, `component:DocsHandoffSection (${routeId})`);
  const finalSource = readRequiredFile(componentPaths.finalCta, `component:FinalCTA (${routeId})`);
  const primitivesSource = readRequiredFile(componentPaths.primitives, `styles:primitives (${routeId})`);
  ...
  console.log('[accessibility foundation] Keyboard and semantic foundation checks passed for route "/".');
} catch (error) {
  const message = error instanceof Error ? error.message : String(error);
