...
function validateHeroContract(heroSource, snapshot) {
  const heroPrimaryCount = countMatches(heroSource, /data-action-tier=["']primary["']/g);
  const heroSecondaryCount = countMatches(heroSource, /data-action-tier=["']secondary["']/g);
  const heroSupportingCount = countMatches(heroSource, /data-supporting-path=["']true["']/g);
...
function validateFinalCtaContract(finalCtaSource, snapshot) {
  const finalPrimaryCount = countMatches(finalCtaSource, /data-action-tier=["']primary["']/g);
  const finalSecondaryCount = countMatches(finalCtaSource, /data-action-tier=["']secondary["']/g);
  const finalSupportingCount = countMatches(finalCtaSource, /data-supporting-path=["']true["']/g);
...
function validateDocsHandoffContract(docsHandoffSource, snapshot) {
  const handoffSecondaryCount = countMatches(
    docsHandoffSource,
    /class=["'][^"']*handoff-link[^"']*["']/g,
  );
  const handoffTierCount = countMatches(docsHandoffSource, /data-action-tier=["']secondary["']/g);
  const handoffSupportingCount = countMatches(
    docsHandoffSource,
    /data-supporting-path=["']true["']/g,
  );
