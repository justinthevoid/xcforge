...
function relFromWebsite(filePath) {
...
function writeRemediationArtifact() {
	...
	const markdownLines = [
		...
		'',
		'Generated by `website/scripts/validate-assistive-tech-contrast-motion.mjs`.',
		'',
...
function reportAndExitIfFailures() {
	...
	failures.forEach((failure, index) => {
		console.error(
			`${index + 1}. [${failure.scope}] ${relFromWebsite(failure.filePath)} :: ${failure.message}`,
		);
